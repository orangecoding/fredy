 ...
 * 15) Look up travel times to the user's addresses
 * 16) Apply the job's commute filter to the entries that are too far away
 * 17) Dispatch notifications
 *
...
class FredyPipelineExecutioner {
  ...
  async _fetchDetails(newListings) {
    ...
    for (const [index, listing] of listingsToEnrich.entries()) {
      // A run that expanded its search can find a hundred listings at once, and a hundred detail
      // requests as fast as the network allows is the behaviour that earns a block - which leaves
      // the rest of the batch without what only the detail page holds. The provider knows its own
      // host's tolerance; the first request needs no breath before it.
