 ...
 * 14) Measure each listing against what a square metre costs around it
 * 15) Read each listing for the marks of a rental scam
 * 16) Look up travel times to the user's addresses
 * 17) Apply the job's commute filter to the entries that are too far away
 * 18) Dispatch notifications
 *
...
class FredyPipelineExecutioner {
  ...
  execute() {
    return (
      Promise.resolve(urlModifier(this._providerConfig.url, this._providerConfig.sortByDateParam))
        ...
        // After both soft-deleting filters, so no listing on its way out is ever measured, and
        // before the notification, so the figure is already on the row the user is about to open.
        .then(this._benchmarkMarket.bind(this))
        // After the benchmark, never before it: one of the signals is the listing sitting far under
        // what the area costs, and that comparison does not exist until the step above wrote it.
        .then(this._detectScam.bind(this))
   ...
   * the shared browser instance, with the provider's declared
   * `detailFetchDelayMs` between two of them.
   *
  ...
  async _fetchDetails(newListings) {
    ...
    const { detailFetchDelayMs = 0, detailFetchJitterMs = 0 } = this._providerConfig;
    const enriched = [];
    for (const [index, listing] of listingsToEnrich.entries()) {
      // Fast sequential responses can still exceed the provider's request rate.
      if (index > 0 && detailFetchDelayMs > 0) {
  ...
  async _geocode(newListings) {
    // Per listing rather than per batch, even though the batch is one provider's: a provider
    // covering several markets narrows the search to the country this advert's own link names, and
    // a street name that exists in two of them would otherwise be resolved in whichever the
    // geocoder ranked first. The lookup itself reads a cached provider list, so asking per listing
    // costs nothing.
    for (const listing of newListings) {
      if (listing.address && !isLocated(listing)) {
        const countries = await getCountriesForListing(this._providerId, listing);
  ...
  /**
   * Work out what a square metre costs around each new listing.
   *
   * Reads only rows Fredy already has, so it costs no request and cannot fail on anything outside
   * the process. It is still wrapped: a benchmark is a nicety next to the notification, and a
   * broken median must never be the reason a listing does not reach the user.
   *
   * @param {ParsedListing[]} listings
   * @returns {ParsedListing[]} The same listings, with the figures attached where one was found.
   * @private
   */
  _benchmarkMarket(listings) {
    if (listings.length === 0) return listings;
    try {
      applyMarketBenchmark(this._jobKey, listings);
    } catch (err) {
      logger.warn('Could not compute the price per square metre benchmark for the new listings', err);
    }
    return listings;
  }

  /**
   * Read each new listing for the marks of a rental scam.
   *
   * Reads the row and nothing else, so like the benchmark above it costs no request. Wrapped for the
   * same reason: a warning is worth having, and never worth losing the notification over.
   *
   * @param {ParsedListing[]} listings
   * @returns {ParsedListing[]} The same listings, with the signals attached where any fired.
   * @private
   */
  _detectScam(listings) {
    if (listings.length === 0) return listings;
    try {
      applyScamSignals(listings);
    } catch (err) {
      logger.warn('Could not check the new listings for scam signals', err);
    }
    return listings;
  }
