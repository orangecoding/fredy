 ...
 * @property {number} [publishedAt] When the portal says the advert was published or last edited,
 *   as epoch milliseconds. Absent when the portal does not say - the listing then keeps whatever
 *   the row's `created_at` says, which is what the ordering falls back to.
 ...
 * @property {number} [market_median_sqm] Optional median price per square metre around this listing,
 *   attached by the pipeline after the row is stored. Snake case because it is the column name: the
 *   same field arrives on a listing read back out of the database, and two spellings of one figure
 *   is how a card ends up reading one of them and finding nothing.
 * @property {number} [market_sample_size] Optional number of listings that median was taken from.
 * @property {number} [market_radius_km] Optional radius, in kilometres, those listings were found in.
 */
