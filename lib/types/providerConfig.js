 ...
 * @property {number} [detailFetchDelayMs] Pause after each completed detail request except the last, in milliseconds. Defaults to zero.
 ...
 * @property {(url: string) => Promise<number> | number} [activityProbe] Cheap "is this still online?" check for a stored listing. Returns 1 when active, 0 when gone, -1 when the answer could not be obtained (bot wall, rate limit, network failure).
