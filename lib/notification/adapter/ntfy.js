...
Link: ${newListing.link}${fredyLine}`;
    ...
    // ntfy expects HTTP header values to be ASCII. Naively stripping non-ASCII
    // characters would remove German Umlauts (and any other non-ASCII glyphs)
    // from values such as the listing title. Instead we keep CR/LF stripping
    // (to prevent header injection) and, whenever a value still contains
    // non-ASCII characters, encode the whole value as an RFC 2047 base64
    // "encoded-word" which ntfy decodes back to UTF-8.
    // See https://docs.ntfy.sh/publish/#publishing-with-non-ascii-headers
