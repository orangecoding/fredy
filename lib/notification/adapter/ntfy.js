...
/**
 * ntfy expects HTTP header values to be ASCII. Naively stripping non-ASCII characters would remove
 * German Umlauts (and any other non-ASCII glyphs) from values such as the listing title. Instead we
 * keep CR/LF stripping (to prevent header injection) and, whenever a value still contains non-ASCII
 * characters, encode the whole value as an RFC 2047 base64 "encoded-word" which ntfy decodes back
 * to UTF-8. See https://docs.ntfy.sh/publish/#publishing-with-non-ascii-headers
 *
 * @param {any} value
 * @returns {string}
 */
export const sanitizeNtfyHeaderValue = (value) => {
...
Link: ${newListing.link}${fredyLine}`;
    ...
    const sanitizeHeaderValue = sanitizeNtfyHeaderValue;
...
export const sendPriceChange = ({ serviceName, priceChanges, notificationConfig, jobKey, baseUrl }) => {
  ...
  const promises = priceChanges.map((change) => {
    const headers = {
      Title: sanitizeNtfyHeaderValue(priceChangeTitle(change)),
      Priority: sanitizeNtfyHeaderValue(priority),
      Tags: sanitizeNtfyHeaderValue(
        `${serviceName},${jobName},${change.direction === 'down' ? 'chart_with_downwards_trend' : 'chart_with_upwards_trend'}`,
      ),
      Click: sanitizeNtfyHeaderValue(change.link),
    };
    ...
    return fetch(`${server}/${topic}`, {
      ...
      if (!res.ok) throw new Error(`Ntfy message could not be sent. Status code: ${res.status}`);
      return res.text();
    });
  });
...
export const config = {
  id: 'ntfy',
  name: 'ntfy',
  readme: readAdapterReadme('ntfy.md'),
  ...
  fields: {
    ...
    topic: {
      ...
      // The topic itself is the credential here, which is why the server (not the topic) is the target.
      secret: true,
    },
    accessToken: {
      type: 'text',
