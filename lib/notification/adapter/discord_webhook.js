...
const buildEmbed = (jobKey, listing, baseUrl) => {
  ...
  // Only when a router actually answered. An "n/a" field like the ones above would claim the
  // journey was looked at and found wanting, which is not what a missing value means here.
  if (listing.commute) {
    fields.push({
      name: 'Commute',
      value: listing.commute,
      inline: false,
    });
  }
...
export const sendPriceChange = ({ serviceName, priceChanges, notificationConfig, jobKey, baseUrl }) => {
  const adapter = notificationConfig.find((adapter) => adapter.id === config.id);
  const webhookUrl = adapter?.fields?.webhookUrl;
  if (!webhookUrl || priceChanges.length === 0) return Promise.resolve([]);

  const job = getJob(jobKey);
  const jobName = job?.name || jobKey;
  const embeds = priceChanges.map((change) => buildEmbed(jobKey, toPriceChangeListing(change), baseUrl));
  ...
  for (let i = 0; i < embeds.length; i += maxEmbedsPerMessage) {
    ...
    const content =
      i === 0
        ? `*${jobName}:* ${serviceName} reports **${priceChanges.length}** price change(s). ${priceChangeTitle(priceChanges[0])}`
        : '';
    const fetchPromise = fetch(webhookUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content, embeds: embedChunk }),
    }).catch((error) => {
      logger.error(`Error sending Discord price change webhook for chunk starting at ${i}:`, error);
      return Promise.reject(new Error(`Webhook failed: ${error.message}`));
    });
    webhookPromises.push(fetchPromise);
  }
...
export const config = {
  ...
  readme: readAdapterReadme('discord_webhook.md'),
