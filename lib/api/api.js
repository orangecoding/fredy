...
import { authHook, adminHook, SESSION_MAX_AGE } from './security.js';
...
// The cookie rolls on every request (@fastify/session default), so SESSION_MAX_AGE acts as an
// idle timeout here. security.js owns the value and applies the same window to its own expiry
// check, which keeps the cookie and the authorization check in agreement.
