...
import { proxyAuthHook } from './proxyAuth.js';
...
import listingAttachmentsPlugin from './routes/listingAttachmentsRouter.js';
...
import mcpOAuthGrantsPlugin from './routes/mcpOAuthGrantsRoute.js';
...
// Reverse-proxy sign-in (off by default). Global rather than per route group so the public
// login probe and the protected API agree on who is signed in; see proxyAuth.js.
fastify.addHook('preHandler', proxyAuthHook);
...
fastify.register(async (app) => {
  ...
  // Same prefix, separate plugin: the attachment routes take raw file bytes, and a content type
  // parser registered inside its own plugin cannot reach the JSON routes next door.
  app.register(listingAttachmentsPlugin, { prefix: '/api/listings' });
  ...
  app.register(mcpOAuthGrantsPlugin, { prefix: '/api/user/mcp-oauth-grants' });
