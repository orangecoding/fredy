...
import { authHook, adminHook, SESSION_COOKIE_MAX_AGE } from './security.js';
...
// Security headers.
//
// CSP used to be switched off entirely "to avoid breaking the SPA". The app has three
// dangerouslySetInnerHTML sites, so a policy is the cheap mitigation worth having. Every allowance
// below is something the SPA demonstrably does today; the value of the policy is in what stays
// closed - no third-party script origin, no eval, no framing, no form posting elsewhere.
await fastify.register(fastifyHelmet, {
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      scriptSrc: ["'self'"],
      // The map popups are built as an HTML string and wired up with onclick/onerror attributes
      // (see ui/src/views/listings/Map.jsx). Helmet's default `script-src-attr 'none'` blocks
      // those outright, which silently kills the "view details" and "delete" buttons on every
      // marker and the broken-image fallback. This does not permit inline <script> - scriptSrc
      // above still governs that.
      scriptSrcAttr: ["'unsafe-inline'"],
      // MapLibre compiles its worker from a blob.
      workerSrc: ["'self'", 'blob:'],
      childSrc: ["'self'", 'blob:'],
      // Semi UI writes inline styles for its themes and popovers; the shell loads Outfit and
      // JetBrains Mono from Google Fonts.
      styleSrc: ["'self'", "'unsafe-inline'", 'https://fonts.googleapis.com'],
      fontSrc: ["'self'", 'data:', 'https://fonts.gstatic.com'],
      imgSrc: ["'self'", 'data:', 'blob:', 'https:'],
      // The two tile sources the map actually fetches from (street + satellite). Geocoding runs
      // server-side, so the browser never talks to Nominatim itself.
      connectSrc: ["'self'", 'https://tiles.openfreemap.org', 'https://server.arcgisonline.com'],
      objectSrc: ["'none'"],
      frameAncestors: ["'none'"],
      baseUri: ["'self'"],
      formAction: ["'self'"],
...
//
// `secure` follows the configured baseUrl: on an https deployment the cookie must not be sent
// over a plaintext downgrade, and on the common http-behind-a-proxy setup it has to stay off or
// nobody can log in.
...
await fastify.register(fastifySession, {
  ...
  cookie: {
    ...
    httpOnly: true,
    secure: typeof settings.baseUrl === 'string' && settings.baseUrl.startsWith('https://'),
    sameSite: 'lax',
  },
