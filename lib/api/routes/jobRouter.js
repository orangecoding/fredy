...
import { sanitiseNotificationAdapter, privilegedFieldError } from '../../services/security/adapterFields.js';
...
export default async function jobPlugin(fastify) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    try {
      ...
      if (!sanitised.ok) {
        return reply.code(403).send({ error: privilegedFieldError(sanitised.adapterId, sanitised.field) });
      }
