...
import {
  getLatestListingsForJobIds,
  getListingActivityPerJob,
 ...
 *
 * Paused jobs included. The dashboard lists every job the user has (its jobs panel reads the full
 * job list), and a paused job's listings are still on the listings page; leaving them out showed a
 * paused job as "nothing found", and a user whose jobs were all paused the "create your first job"
 * screen. Only the scheduler status is about the jobs that actually run, see `computeLastRun`.
 *
 * @param {import('fastify').FastifyRequest} request
 * @returns {Object[]}
 */
function getAccessibleJobs(request) {
  return jobStorage.getJobs({ includeDisabled: true }).filter((job) => canAccessJob(request.currentUser, job));
}
...
export default async function dashboardPlugin(fastify) {
  fastify.get('/', async (request) => {
    ...
    const {
      ...
      medianPriceSampleSize,
      medianPricePerSqm,
    } = getListingsKpisForJobIds(jobIds);
    ...
    // A paused job is not waiting for its next run, so it says nothing about the scheduler.
    const lastRun = computeLastRun(jobs.filter((job) => job.enabled !== false));
    ...
    return {
      ...
      pie: providerPie,
      latest: getLatestListingsForJobIds(jobIds, 8),
      jobActivity: getListingActivityPerJob(jobIds, 7),
    };
  });
}
