...
export default async function userSettingsPlugin(fastify) {
  ...
  fastify.post('/home-address', async (request, reply) => {
    ...
    try {
      ...
      if (entries.length === 0) {
        // Clear both the new list and the legacy single field so it can't resurface.
        upsertSettings({ home_addresses: null, home_address: null }, userId);
      ...
      // Geocode each address; failures are stored with the {-1,-1} sentinel so
      // the UI can flag the offending row (same pattern as the old single field).
      const geocoded = [];
      ...
      upsertSettings({ home_addresses: geocoded, home_address: null }, userId);
      resetGeocoordinatesAndDistanceForUser(userId);
      runGeoCordTask();
