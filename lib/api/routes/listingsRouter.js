...
export default async function listingsPlugin(fastify) {
  ...
  fastify.post('/:listingId/scam', async (request, reply) => {
    ...
    try {
      ...
      // Only the two verdicts are counted. Clearing one is somebody undoing a click, and counting
      // that as a third kind of usage would inflate the very number it is meant to measure.
      if (normalized != null) {
