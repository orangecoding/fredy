...
import {
  cancelManualAppointmentForListing,
  completeManualAppointmentForListing,
  saveManualAppointment,
} from '../../services/storage/manualAppointmentStorage.js';
import { LISTING_STATUSES, LISTING_STATUS_FILTERS } from '../../services/storage/listingStatus.js';
...
export default async function listingsPlugin(fastify) {
  fastify.get('/table', async (request) => {
    ...
    const normalizedStatus =
      typeof statusFilter === 'string' && LISTING_STATUS_FILTERS.includes(statusFilter.toLowerCase())
  ...
  fastify.post('/:listingId/status', async (request, reply) => {
    const { listingId } = request.params || {};
    const { status, appointmentAt = null, timezone = 'Europe/Berlin' } = request.body || {};
    const userId = request.session?.currentUser;
    if (!listingId || !userId) {
      return reply.code(400).send({ message: 'listingId or user not provided' });
    }
    ...
    if (normalized === 'invited' && (!Number.isFinite(Number(appointmentAt)) || Number(appointmentAt) <= 0)) {
      return reply.code(400).send({ message: 'Appointment date and time are required for invited listings.' });
    }
    try {
      ...
      if (normalized === 'visited') {
        completeManualAppointmentForListing({ listingId, userId });
      }
      if (normalized == null || normalized === 'rejected' || normalized === 'not_invited') {
        cancelManualAppointmentForListing({ listingId, userId });
      }
    } catch (error) {
      logger.error(error);
