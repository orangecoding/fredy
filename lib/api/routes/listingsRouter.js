...
export default async function listingsPlugin(fastify) {
  fastify.get('/table', async (request) => {
    ...
    // request, so a hand-edited or bookmarked URL cannot inject its own price bounds.
    // An incomplete profile yields no band at all, which means the filter is ignored rather
    // than returning an empty page. The settings row is only read when the filter is actually
    // in play - this is the hottest endpoint in the app and most requests do not use it.
    const userId = request.session.currentUser;
    const affordabilityBand = nullOrEmpty(affordabilityFilter)
      ? null
      : affordabilityBandFor(affordabilityFilter, getUserSettings(userId)?.finance_profile ?? null);

    // Each row carries its job's deal type; the UI pairs that with the thresholds it already
    // derives from the profile in its store, so no threshold has to travel with the page.
