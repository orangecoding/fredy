...
/**
 * Bounds for the listing document settings.
 *
 * Uploaded documents live in the database, so these two are the only thing standing between a
 * self-hosted instance and a listings.db that quietly grows until the backup stops fitting
 * anywhere. The size ceiling is 50 rather than something rounder because `lib/api/api.js` caps
 * request bodies at 50 MB: a larger value could only ever produce a 413 from Fastify before any
 * handler runs, which would look like a broken feature rather than a setting out of range.
 * @type {Record<string, {min: number, max: number}>}
 */
const ATTACHMENT_SETTING_BOUNDS = {
  listingAttachmentMaxMb: { min: 1, max: 50 },
  listingAttachmentMaxPerListing: { min: 1, max: 200 },
};

/**
 * Validate one numeric listing document setting.
 *
 * @param {string} name The setting name, used in the message.
 * @param {any} value The raw value from the request body.
 * @returns {string|null} An error message, or null when the value is acceptable.
 */
function validateAttachmentSetting(name, value) {
  const bounds = ATTACHMENT_SETTING_BOUNDS[name];
  if (value === '' || value === null) {
...
export default async function generalSettingsPlugin(fastify) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    for (const name of Object.keys(ATTACHMENT_SETTING_BOUNDS)) {
      if (typeof appSettings[name] === 'undefined') continue;
      const error = validateAttachmentSetting(name, appSettings[name]);
      if (error != null) {
        return reply.code(400).send({ error });
      }
