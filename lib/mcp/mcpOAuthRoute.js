...
async function mcpOAuthPlugin(fastify) {
  ...
  fastify.get('/api/oauth/authorize', async (request, reply) => {
    ...
    return reply
      .type('text/html')
      .send(
        `<!doctype html><title>Authorize Claude</title><main><h1>Authorize access</h1><p>Allow this client to read your Fredy listings and jobs?</p><form method="post"><input type="hidden" name="response_type" value="code"><input type="hidden" name="client_id" value="${escapeHtml(authorization.params.client_id)}"><input type="hidden" name="redirect_uri" value="${escapeHtml(authorization.params.redirect_uri)}"><input type="hidden" name="state" value="${escapeHtml(authorization.params.state || '')}"><input type="hidden" name="code_challenge" value="${escapeHtml(authorization.params.code_challenge)}"><input type="hidden" name="code_challenge_method" value="S256"><input type="hidden" name="resource" value="${escapeHtml(authorization.urls.resource)}"><input type="hidden" name="scope" value="${SCOPE}"><button type="submit">Allow</button></form></main>`,
      );
  });
