...
/**
 * Reading is the floor: a token that cannot read is of no use to an MCP client, and
 * `validateAccessToken` refuses one.
 */
const READ_SCOPE = 'mcp:read';

/**
 * Writing is asked for separately, and shown separately on the consent page.
 *
 * Write tools arrived after remote connectors did. Every grant approved before them was approved on
 * a page that said "read"; letting those tokens create jobs and rewrite notes would be an upgrade
 * the user never agreed to. A connection made before this scope existed keeps working, read-only,
 * until its owner reconnects and sees the new page.
 */
...
async function authorizationRequest(request) {
  ...
  // Whatever the client asked for, narrowed to what this server actually issues. A request naming
  // a scope Fredy does not know is not refused - it is simply not granted, which is what keeps a
  // client that probes for extra scopes from being locked out entirely.
  const requested = String(params.scope || '')
    .split(' ')
    .filter(Boolean);
  const scopes = SCOPES.filter((scope) => requested.includes(scope));
  if (
    ...
    return { error: 'invalid_request' };
  return { params, client, urls, scopes };
}
...
async function mcpOAuthPlugin(fastify) {
  ...
  fastify.get('/.well-known/oauth-protected-resource/api/mcp', async () => {
    const urls = await oauthUrls();
    return { resource: urls.resource, authorization_servers: [urls.baseUrl], scopes_supported: SCOPES };
  });
    ...
    return reply.type('text/html').send(
      renderPage({
        ...
        body: `${BRAND}<h1>Authorize access</h1><p class="lead"><strong>${escapeHtml(
          clientName,
        )}</strong> wants to connect to your Fredy account.</p><ul class="scopes"><li>${CHECK}Read your saved property listings</li><li>${CHECK}Read your search jobs and their results</li>${
          authorization.scopes.includes(WRITE_SCOPE)
            ? `<li>${CHECK}Create search jobs, add notes and manage your watchlist</li>`
            : ''
        }</ul><form method="post"><input type="hidden" name="response_type" value="code"><input type="hidden" name="client_id" value="${escapeHtml(
        ...
        )}"><input type="hidden" name="scope" value="${escapeHtml(authorization.scopes.join(' '))}"><button type="submit" class="btn btn-allow">Allow access</button></form>${
          returnHost
