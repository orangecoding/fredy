...
export function createMcpServer() {
  ...
  server.tool(
      ...
      'Use get_listing to get full details (description, link, image) for a specific listing.',
    {
      ...
      status: z
        .enum(['applied', 'invited', 'visited', 'documents_sent', 'accepted', 'rejected', 'not_invited', 'none'])
        .optional()
        .describe('Filter by user-set application status; "none" returns only listings without a status set.'),
    },
    async (
      {
        page,
