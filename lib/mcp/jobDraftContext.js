...
export async function buildDraftContext(user) {
  // `hosts` is carried along rather than dropped: immowelt serves `.de` and `.at` and idealista
  // three domains, and a URL on one of the others is only resolvable to its provider if the list
  // gets this far. Narrowing to `baseUrl` here is what refused an Austrian search the web form took.
  const providers = (await getProviders())
    .map((provider) => provider.metaInformation)
    .filter((meta) => meta?.id != null && meta?.baseUrl != null)
    .map(({ id, name, baseUrl, hosts }) => ({ id, name, baseUrl, ...(hosts == null ? {} : { hosts }) }));
