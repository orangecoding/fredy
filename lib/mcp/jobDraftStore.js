...
import { hostsOf, normalizeHost, validateProviderUrl } from '../services/jobs/providerUrl.js';
 ...
 * @property {Array<{id: string, name: string, baseUrl: string, hosts?: string[]}>} providers - Every
 *   portal Fredy can crawl, as `metaInformation`. `hosts` has to survive the trip: a provider
 *   serving several countries as several domains is only recognisable by all of them.
 ...
 *
 * The host is the only part of a search URL that identifies the portal. Asking the user to name the
 * portal as well as paste the URL would be asking them for something the URL already says.
 *
 * Every host the provider declares, not just `baseUrl`'s: immowelt serves `.de` and `.at`, idealista
 * `.com`, `.it` and `.pt`, and a search on the Austrian or Italian site is a search this provider
 * can perfectly well run. Matching on `baseUrl` alone refused them with "unknown portal host", while
 * the web form accepted the very same URL.
 *
 * @param {string} url
 * @param {Array<{id: string, name: string, baseUrl: string, hosts?: string[]}>} providers
 * @returns {{id: string, name: string, baseUrl: string, hosts?: string[]}|null}
 */
export function resolveProviderForUrl(url, providers) {
  const host = normalizeHost(url);
  if (host == null) return null;
  return (providers ?? []).find((provider) => hostsOf(provider).includes(host)) ?? null;
}

const supportedHostList = (ctx) =>
  (ctx.providers ?? []).map((provider) => `${provider.name} (${hostsOf(provider).join(', ')})`).join(', ');
...
export function applyDraftUpdate(draft, patch, ctx) {
  ...
  if (patch.providerUrls !== undefined) {
    ...
    const resolved = [];
    // Against this block's own problems, not the running total. Reading the shared list made the
    // assignment depend on whether an earlier field happened to fail, which is invisible here and
    // only harmless as long as the whole patch is rejected together.
    ...
    if (problems.length === problemsBefore) next.providers = resolved;
  }
  ...
  if (patch.channelIds !== undefined) {
    ...
    if (problems.length === problemsBefore) next.channelIds = ids;
  }
  ...
  if (patch.shareWithUsers !== undefined) {
    ...
    if (problems.length === problemsBefore) next.shareWithUsers = ids;
  }

  if (patch.enabled !== undefined) {
    // The one refinement that carries a default, and so the one a model sets without being asked:
    // "create a search called X, switched on" arrives as name and enabled in a single call. Every
    // other refinement in this list is something the user volunteered or explicitly declined, but
    // taking `enabled` as an answer to a question nobody had put yet marked the whole step done -
    // and price, size, rooms, blacklist, commute and sharing were never asked about at all. It
    // counts only once the interview has actually reached that question.
    if (missingRequired(draft).length === 0) touchedRefinement = true;
...
export function describeDraft(draft, ctx) {
  ...
  const lines = [
    `- **Name:** ${draft.name ?? '–'}`,
    // Unanswered is its own state, not renting. This summary is what the model reads back to the
    // user before they confirm, and printing the fallback made it confirm a deal type nobody had
    // been asked for - then ask for it on the very next turn.
    `- **Deal type:** ${draft.dealType == null ? '–' : draft.dealType === DEAL_TYPES.BUY ? 'buying' : 'renting'}`,
