 ...
 * @property {string[]} scopes - What this client may currently do. The union across the grant's
 *   live refresh tokens: if any one of them carries `mcp:write`, the client can obtain a token
 *   that writes, and saying otherwise on the settings page would be a lie.
 */
...
export function listGrants(userId) {
  const rows = SqliteConnection.query(
    `SELECT c.id AS clientId, c.name AS clientName, MIN(t.created_at) AS grantedAt,
            GROUP_CONCAT(t.scopes, '|') AS scopeLists
  ...
  return rows.map(({ scopeLists, ...grant }) => ({ ...grant, scopes: parseScopeLists(scopeLists) }));
}

/**
 * The distinct scopes across a grant's refresh tokens.
 *
 * `GROUP_CONCAT` is the only way to fold a JSON column across a GROUP BY, so the rows arrive as
 * `["mcp:read"]|["mcp:read","mcp:write"]`. The pipe is safe as a separator: a scope is a bare
 * token and JSON escapes nothing to it.
 *
 * A row that will not parse is skipped rather than thrown: this feeds a settings page, and one bad
 * row must not be the reason somebody cannot see - or revoke - the rest of their connections.
 *
 * @param {string|null} scopeLists
 * @returns {string[]}
 */
function parseScopeLists(scopeLists) {
  const scopes = new Set();
  for (const list of String(scopeLists ?? '').split('|')) {
    if (list.length === 0) continue;
    try {
      for (const scope of JSON.parse(list)) scopes.add(scope);
    } catch {
      // Not parseable, so it says nothing about what the client may do.
    }
  }
  return [...scopes].sort();
}
