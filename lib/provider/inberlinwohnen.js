...
async function fetchDetails(listing, browser, extractDetails = puppeteerExtractor) {
  try {
    ...
    const fullDescription = [listing.description, ...details].filter(Boolean).join('\n\n');
...
function normalize(o) {
  ...
  // The rest of Fredy reads `price` as a cold rent: the affordability check adds the Nebenkosten
  // surcharge itself, so quoting the Gesamtmiete here counted them twice and made every listing
  // from this provider look 25 % more expensive than it is. Falls back to the total for the few
  // listings that publish no cold rent, because that is the only number they publish.
  const coldRent = firstScalarValue(item.rentNet) ?? totalRent;
...
const config = {
  ...
  // The Wohnungsfinder is a single unfiltered page; the URL carries no search of any kind.
