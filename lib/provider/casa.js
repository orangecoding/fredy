 ...
 *
 * What is left for the page is a url that cannot be read into one: a filter with no counterpart, a
 * category outside the confirmed table, a place the lookup does not know. The page goes through the
 * shared browser rather than through `fetch`, because casa.it sits behind DataDome and answers a
 * plain request with a 403 interstitial, and a render that comes back walled anyway falls through
 * to the service `FREDY_CHALLENGE_SOLVER_URL` names.
 */
