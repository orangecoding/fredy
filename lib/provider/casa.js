...
/** The zone casa.it writes its stamps in, whatever the `Z` on them says. */
const CASA_ZONE = 'Europe/Rome';

/**
 * Read one of the api's compact timestamps into epoch milliseconds.
 *
 * `Date.parse` refuses the format, so it is read by hand. The trailing `Z` is decoration: a search
 * answered at 18:38 UTC on a summer afternoon carries adverts stamped 19:42, an hour into the
 * future, and the stamps of a whole page cluster over an Italian office day rather than over one
 * shifted two hours earlier. What the api writes is a Rome wall clock, so that is how it is read -
 * otherwise every advert edited within the last couple of hours is dated into the future.
 *
...
function casaTimestamp(value) {
  ...
  return wallClockInZone(
    { year: Number(y), month: Number(m), day: Number(d), hour: Number(h), minute: Number(min), second: Number(s) },
    CASA_ZONE,
  );
}
