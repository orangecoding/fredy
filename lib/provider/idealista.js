 ...
 * idealista.pt are served by the same application, translated, and the app behind them talks to one
 * api per country - same client key, same signing, same endpoints with the country code swapped.
 * Which of them a job speaks to is read off the hostname of the url it carries
 * (`lib/services/idealista/portal.js`); a url on any other host is refused with a log line rather
 * than guessed at.
 *
 * A search is read through that mobile api, which serves JSON to a plain request: no bot wall, no
 * browser, no challenge solver, and an ordering by publication date that the website's robots.txt
 * refuses. The job still holds the search url a user copied out of their browser, so
 * `lib/services/idealista/` translates that url into the terms the api searches by - first by
 * asking idealista's own parser, then, where that cannot be reached, by reading the url locally.
 *
 * Not every url can be translated - a filter the api has no name for, a category it does not
 * serve - and those searches are read off the website instead, the slow way: behind the challenge
 * solver where one is configured, and otherwise through the run's own browser, waiting out
 * DataDome's interstitial. See `lib/services/idealista/search-filters.js` for what carries over and
 * what does not.
 */
 ...
 * @param {any} [browser] the shared browser of the current job run, which reads the website when no
