...
async function getListings(url) {
  ...
  try {
    ...
    // Log our proxy exit IP for debugging
    try {
      const ipCheckPage = await browser.newPage();
      await ipCheckPage.goto('https://httpbin.org/ip', { timeout: 15_000 });
      const ipText = await ipCheckPage.evaluate(() => document.body.innerText);
      logger.info(`ImmoScout24.ch: Our proxy exit IP: ${ipText.trim()}`);
      await ipCheckPage.close();
    } catch (e) {
      logger.warn('ImmoScout24.ch: Could not check proxy IP');
    }
    ...
    while (attempts < maxAttempts) {
      ...
      if (captchaResult.detected) {
        ...
        if (!solveResult.success || !solveResult.cookie) {
          // If it's an interstitial, just reload and continue - it might pass on retry
          if (solveResult.error?.includes('Interstitial')) {
            logger.info('ImmoScout24.ch: Interstitial detected, reloading page...');
            await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 60_000 });
            await applyPostNavigationHumanSignals(page, preCfg);
            continue;
          }
