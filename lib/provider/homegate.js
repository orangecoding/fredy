...
const config = {
  ...
  puppeteerOptions: {
    ...
    // Homegate uses a virtual list: only cards near the viewport are in the DOM.
    // autoScroll collects card outerHTML at each scroll position so no card is
    // missed regardless of whether it gets unmounted after scrolling past.
    autoScroll: true,
    autoScrollDelay: 800,
    autoScrollItemSelector: 'div[data-test="result-list-item"]',
    // Dedup each card by its listing link href so the same card captured at different
    // scroll positions (where React may render slightly different outerHTML) is only
    // collected once.
