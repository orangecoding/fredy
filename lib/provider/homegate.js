 ...
 *
 * Uses Bright Data Web Unlocker to bypass Cloudflare bot protection.
...
function extractInitialStateFromHtml(html) {
  try {
    ...
    if (!match) {
      logger.warn('Homegate: Could not find __INITIAL_STATE__ in HTML');
      return null;
    }
    return JSON.parse(match[1]);
  } catch (err) {
    logger.error('Homegate: Error parsing __INITIAL_STATE__:', err.message);
    return null;
  }
}
...
function transformListing(item) {
  ...
  return {
    ...
    link: `https://www.homegate.ch/${offerType}/${listingId}`,
...
async function getListings(url) {
  ...
  if (!apiToken || !zone) {
    logger.warn('Homegate: Bright Data API token or zone not configured. Please add them in Settings.');
    return [];
  }

  try {
    logger.debug(`Homegate: Fetching listings via Bright Data from ${url}`);
    ...
    if (!response.ok) {
      ...
      logger.error(`Homegate: Bright Data API error (${response.status}): ${errorText}`);
      return [];
    }
    ...
    logger.debug(`Homegate: Received ${html.length} bytes from Bright Data`);

    // Extract __INITIAL_STATE__ from HTML
    const initialState = extractInitialStateFromHtml(html);
    if (!initialState) {
      logger.warn('Homegate: Could not extract __INITIAL_STATE__ from response');
      return [];
    }

    // Extract and transform listings
    const rawListings = extractListingsFromState(initialState);
    logger.info(`Homegate: Found ${rawListings.length} listings`);

    return rawListings.map(transformListing);
  } catch (error) {
    if (error.name === 'AbortError') {
      logger.error(`Homegate: Request timed out after ${BRIGHT_DATA_TIMEOUT_MS / 1000}s`);
    } else {
