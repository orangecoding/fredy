...
import { buildHash, isOneOf, randomBetween, sleep } from '../utils.js';
import {
  convertImmoscoutListingToMobileListing,
  convertWebToMobile,
} from '../services/immoscout/immoscout-web-translator.js';
import { MAX_PROBE_ATTEMPTS, backoffDelay, retryAfterDelay } from '../services/listings/activeProbeBackoff.js';
...
/**
 * The price range of an ImmoScout search.
 *
 * ImmoScout hides the price in three places: the `price` query parameter, the SEO path segments
 * (`guenstige-wohnung-mieten` means "up to 400 €", `wohnung-bis-800-euro-warm` means what it says)
 * and the per-type defaults. {@link convertWebToMobile} already resolves all three in the site's own
 * precedence order, so reading its output is both less code than doing it again and guaranteed to
 * describe the search Fredy actually runs.
 *
 * Note that `pricetype` decides whether the bounds are Kaltmiete or Warmmiete. It is not reported -
 * see the tracking payload - so two searches with the same numbers and different price types look
...
function parsePriceRange(url) {
  const price = new URL(convertWebToMobile(url)).searchParams.get('price');
