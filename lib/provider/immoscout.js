...
async function fetchFromImmoScout(url, options = {}) {
  ...
  if (!response.ok) {
    logger.error(`Error fetching data from ImmoScout Mobile API: ${response.status} ${response.statusText} for URL: ${url}`);
    return null;
  }
...
async function getListings(url) {
  const responseBody = await fetchFromImmoScout(url, {
    method: 'POST',
    body: JSON.stringify({
      supportedResultListTypes: [],
      userData: {},
...
async function getExpose(url, headers) {
  const options = {
    headers: headers,
  };
  return fetchFromImmoScout(url, options);
}
...
function getExposeConfig(expose) {
  return {
    url: `https://api.mobile.immobilienscout24.de/expose/${expose.id}`,
    headers: {
      'User-Agent': 'ImmoScout24_1410_30_._',
...
function getExposeSelectors() {
  return [
    `$.sections[?(@.type=="TOP_ATTRIBUTES")].attributes[?(@.label=="Zimmer")].text`,
    `$.sections[?(@.type=="TOP_ATTRIBUTES")].attributes[?(@.label=="Wohnfläche")].text`,
    `$.sections[?(@.type=="TITLE")].title`,
    `$.sections[?(@.type=="MAP")].addressLine1`,
    `$.sections[?(@.type=="MAP")].addressLine2`,
    `$.sections[?(@.type=="ATTRIBUTE_LIST" && @.title=="Hauptkriterien")].attributes`,
    `$.sections[?(@.type=="ATTRIBUTE_LIST" && @.title=="Kosten")].attributes`,
    `$.sections[?(@.type=="ATTRIBUTE_LIST" && @.title=="Bausubstanz & Energieausweis")].attributes`,
    `$.sections[?(@.type=="TEXT_AREA" && @.title=="Ausstattung")].text`,
    `$.sections[?(@.type=="TEXT_AREA" && @.title=="Lage")].text`,
    `$.sections[?(@.type=="TEXT_AREA" && @.title=="Sonstiges")].text`
  ];
}
