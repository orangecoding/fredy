...
async function getListings(url) {
  ...
  // helper: convert media entries (objects or strings) to URL strings
  const toUrl = (m) => {
    if (!m) return null;
    if (typeof m === 'string') return m;
    // prefer fullImageUrl, fallback to web or preview
    return m.fullImageUrl || m.imageUrlForWeb || m.previewImageUrl || null;
  };

  const extractMediaUrlsFromSections = (sections) => {
    if (!Array.isArray(sections)) return [];
    const mediaSections = sections.filter((s) => s && s.type === 'MEDIA');
    const urls = mediaSections.flatMap((s) => (Array.isArray(s.media) ? s.media.map(toUrl) : []));
    return urls.filter(Boolean);
  };
  const basicResults = responseBody.resultListItems
    ...
    .map((expose) => {
      ...
      // normalize initial additionalpictures (could be array of objects or strings)
      const initialAdditionalPictures =
        Array.isArray(item?.pictures) && item.pictures.length > 0
          ? item.pictures
          : item?.titlePicture
            ? [item.titlePicture]
            : [];
      // map initial pictures to URL strings
      const mappedInitialPictures = Array.isArray(initialAdditionalPictures)
        ? initialAdditionalPictures.map(toUrl).filter(Boolean)
        : [];

      // try to extract MEDIA sections already if detailedinfo is embedded
      let extractedAdditionalPictures = mappedInitialPictures;
      try {
        const sections =
          item && item.detailedinfo && Array.isArray(item.detailedinfo.sections) ? item.detailedinfo.sections : null;
        if (sections) {
          extractedAdditionalPictures = extractMediaUrlsFromSections(sections);
          // store on the original item as requested
          try {
            item.additionalpictures = extractedAdditionalPictures;
          } catch {
            /* ignore */
          }
        }
      ...
      return {
        __sourceItem: item, // keep reference for later
        id: item.id,
        price: price?.value,
        size: size?.value,
        title: item.title,
        ...
        image,
        additionalpictures: extractedAdditionalPictures,
        detailedinfo: null,
      };
    });

  // Fetch detailed info for all items in parallel and attach results
  await Promise.all(
    basicResults.map(async (resultObj) => {
      const item = resultObj.__sourceItem;
      try {
        const webLink = `${metaInformation.baseUrl}expose/${item.id}`;
        const mobileUrl = convertImmoscoutListingToMobileListing(webLink);
        const detail = await getdetailedinfo(mobileUrl);
        resultObj.detailedinfo = detail;
        try {
          item.detailedinfo = detail;
        } catch {
          /* ignore */
        }
        // optional debug logging - show the sections array when available
        try {
          if (detail && typeof detail === 'object' && Array.isArray(detail.sections)) {
            const additionalpictures = extractMediaUrlsFromSections(detail.sections);
            if (additionalpictures.length > 0) {
              // persist on both the source item and result object for later use
              try {
                item.additionalpictures = additionalpictures;
              } catch {
                /* ignore */
              }
              resultObj.additionalpictures = additionalpictures;
            }
          }
        } catch (e) {
          // never let extraction/logging break the flow
          logger.debug('Failed to extract MEDIA sections from detailed info', e && e.message);
        }
      } catch (err) {
        logger.debug(`error fetching detailed info for ${item.id}:`, err && err.message);
        resultObj.detailedinfo = null;
      }
      // remove helper pointer before returning
      delete resultObj.__sourceItem;
    }),
  );
...
/**
 * Fetch detailed information for a listing or endpoint using a GET request.
 * The URL can be passed directly to the function or configured via init (config.detailedInfoUrl).
 * Returns parsed JSON when possible, otherwise returns the raw text response.
 *
 * @param {string} [url] optional URL to fetch; falls back to config.detailedInfoUrl or the provider base URL
 * @returns {Promise<any|null>} parsed response or null on non-OK responses
 */
async function getdetailedinfo(url) {
  const fetchUrl = url || metaInformation.baseUrl;
  try {
    ...
    if (!res.ok) {
      logger.error('Error fetching detailed info from ImmoScout Mobile API:', res.statusText, fetchUrl);
      return null;
    }
    ...
    logger.error('Exception fetching detailed info:', err && err.message, url);
    return null;
  }
}
