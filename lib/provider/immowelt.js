...
import { IMMOWELT_HOSTS, requireSite, siteOf } from '../services/immowelt/site.js';
 ...
 *
 * Which of immowelt's sites the search runs against is read off the job's own url: the two are one
 * application, and the only thing that differs is the origin every request goes to.
 *
...
async function getListings(url, browser) {
  // The site is resolved first so that a url on no immowelt host is named as such, rather than
  // reported as whatever the search translator makes of a stranger's query string.
...
function countryOf(listing) {
  return siteOf(listing?.link)?.country ?? null;
}
