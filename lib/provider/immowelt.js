...
/**
 * The exposé elements holding description parts.
 *
 * The card payload truncates it at 200-500 characters, which is enough to read but not enough to
...
const ADDITIONAL_DESCRIPTION_SELECTOR = '[data-testid="cdp-additional-description-expandable-text"]';

/**
 * Extract the full, unshortened description text from immowelt exposé HTML.
 * Attempts to read the structured server state (__UFRN_LIFECYCLE_SERVERREQUEST__) first,
 * and falls back to Cheerio DOM selectors.
 *
 * @param {string} html
 * @returns {string|null}
 */
export function extractExposeDescription(html) {
  ...
  // 1. Structured JSON state from __UFRN_LIFECYCLE_SERVERREQUEST__
  try {
    const match = html.match(/__UFRN_LIFECYCLE_SERVERREQUEST__.*?JSON\.parse\("(.*?)"\);/s);
    if (match) {
      const unescaped = JSON.parse(`"${match[1]}"`);
      const parsed = JSON.parse(unescaped);
      const sections = parsed?.app_cldp?.data?.classified?.sections;

      // Check sections.description.texts (structured array of { headline, text })
      const texts = sections?.description?.texts;
      ...
      // Fall back to individual section descriptions
      const parts = [
        sections?.mainDescription?.description,
        sections?.areaDescription?.description,
        sections?.extendedInfoDescription?.description,
      ]
        .filter(Boolean)
    ...
    // fall back to DOM selectors below
  }

  // 2. DOM selectors with Cheerio
  ...
  const additionalDescription = $(ADDITIONAL_DESCRIPTION_SELECTOR).first().text().trim();

  const full = [description, locationDescription, additionalDescription].filter(Boolean).join('\n\n');
  return full || null;
}
...
async function fetchDetails(listing, browser) {
  try {
    ...
    const full = extractExposeDescription(html);
