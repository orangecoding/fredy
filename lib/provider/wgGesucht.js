...
function normalize(o) {
  ...
  const [rooms, ciity, road] = o.tags.split(' | ');
  return {
    id,
    link,
    title: o.title || '',
...
const config = {
  ...
  crawlFields: {
    ...
    rooms: '.middle .text-right |removeNewline |trim', // Same selector often contains both, will be filtered by extractNumber
    title: '.truncate_title a |removeNewline |trim',
    link: '.truncate_title a@href',
    image: '.img-responsive@src',
    tags: '.row .noprint .col-xs-11 |removeNewline |trim',
