 ...
 *
 * That endpoint only answers a search carrying a short-lived token, which the website fetches
 * from GET /api/real-estate/search-token first and sends as `X-VON-Search-Token`.
 *
...
/**
 * Where the website fetches the token the list endpoint insists on.
 *
 * A search without one is answered `401 {"error":"Unauthorized"}` - by the application itself, not
 * by a bot filter - which took the provider out entirely the day the token appeared. It lives
 * fifteen minutes (`ttl: 900`) and names no search, so each run fetches its own rather than
 * keeping one at module scope.
 */
const SEARCH_TOKEN_PATH = 'api/real-estate/search-token';

/** The header the website sends the token in. */
const SEARCH_TOKEN_HEADER = 'X-VON-Search-Token';

/**
 * Fetch a search token the way the website does before it searches.
 *
 * Answers null rather than failing. The website sends its search without the header when the
 * endpoint gives it nothing, and so does the provider, so it keeps working should Deutsche Wohnen
 * drop the requirement again.
 *
 * @param {Object.<string, string>} headers The run's request headers.
 * @returns {Promise<string|null>}
 */
export async function fetchSearchToken(headers) {
  try {
    const response = await fetch(`${metaInformation.baseUrl}${SEARCH_TOKEN_PATH}`, { method: 'GET', headers });
    if (!response.ok) {
      logger.error('Could not obtain a Deutsche Wohnen search token:', response.statusText);
      return null;
    }
    const { token } = await response.json();
    return typeof token === 'string' && token.length > 0 ? token : null;
  } catch (error) {
    logger.error('Could not obtain a Deutsche Wohnen search token:', error?.message || error);
    return null;
  }
}
...
async function getListings(url) {
  ...
  const searchHeaders = token ? { ...headers, [SEARCH_TOKEN_HEADER]: token } : headers;
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    const response = await fetch(pageUrl.toString(), { method: 'GET', headers: searchHeaders });
    if (!response.ok) {
...
const config = {
  ...
  url: null,
  // Deutsche Wohnen's own filter, read off a search with both bounds set:
