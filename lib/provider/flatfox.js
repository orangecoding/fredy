...
async function fetchListingIds(params) {
  ...
  // Rooms - website uses min_rooms/max_rooms, API uses rooms_from/rooms_to
  const roomsFrom = params.rooms_from || params.min_rooms;
  const roomsTo = params.rooms_to || params.max_rooms;
  if (roomsFrom) apiUrl.searchParams.set('rooms_from', roomsFrom);
  if (roomsTo) apiUrl.searchParams.set('rooms_to', roomsTo);

  // Price - website uses min_price/max_price, API uses price_from/price_to
  const priceFrom = params.price_from || params.min_price;
  const priceTo = params.price_to || params.max_price;
  if (priceFrom) apiUrl.searchParams.set('price_from', priceFrom);
  if (priceTo) apiUrl.searchParams.set('price_to', priceTo);
 ...
 * The pin API doesn't respect price/rooms filters, so we filter client-side.
 */
function filterListings(listings, params) {
  const minRooms = parseFloat(params.rooms_from || params.min_rooms || 0);
  const maxRooms = parseFloat(params.rooms_to || params.max_rooms || Infinity);
  const minPrice = parseFloat(params.price_from || params.min_price || 0);
  const maxPrice = parseFloat(params.price_to || params.max_price || Infinity);
