...
async function getListings(url) {
  try {
    ...
    logger.debug(`Flatfox: Pin API returned ${pks.length} PKs`);
    ...
    logger.debug(`Flatfox: Details API returned ${listings.length} listings`);

    // Filter by price/rooms (pin API doesn't support these filters)
    const filtered = filterListings(listings, params);
    logger.debug(`Flatfox: After price/rooms filter: ${filtered.length} listings`);
