...
/**
 * Kleinanzeigen keeps its price filter in the path, not the query string:
 * `/s-immobilien/duesseldorf/anzeige:angebote/preis:500:1000/wohnung/k0c195l2068r5`.
 *
 * The one-sided forms (`preis:500:` and `preis::1000`) follow the same segment shape, which is why
 * both halves are optional here. An unrecognised spelling falls through to "no range" rather than
 * to half a range, so a filter this misses costs an unreported band and never a wrong one.
 *
 * @param {string} url The job's search url.
 * @returns {{min: string|null, max: string|null}}
 */
function parsePriceRange(url) {
  const match = /(?:^|\/)preis:([^:/]*):([^:/]*)(?:\/|$)/.exec(new URL(url).pathname);
