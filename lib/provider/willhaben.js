  ...
  return readAdverts(await response.text())
    ...
    .map((advert) => {
      ...
      return {
        ...
        publishedAt: publicationDate(Number(attributes.PUBLISHED)),
...
function normalize(o) {
  return {
    ...
    publishedAt: publicationDate(o.publishedAt),
    image: o.image,
  };
}
...
const config = {
  ...
  // Unverified: willhaben was unreachable when the other portals' parameters were recorded, so
  // nobody has seen a filtered search URL of theirs. Null reports no band rather than a guessed
  // one; fill this in once a real URL can be looked at.
