 ...
 */
function exposeLink(id) {
  if (!id) return null;
  try {
    const searchUrl = new URL(config.url ?? `${BASE_URL}/de/de/propertysearch`);
...
/**
 * Enriches a listing with the exposé and location text. The search result page carries the
 * headline only, so without this the description stays empty.
 *
