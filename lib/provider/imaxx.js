...
/**
 * Read the publication date of this listing's page, ignoring other JSON-LD entities.
 * @param {import('cheerio').CheerioAPI} $ The detail document.
 * @param {string} link The listing URL.
 * @returns {number|undefined} Epoch milliseconds, when the page declares a valid date.
 */
function readPublicationDate($, link) {
  for (const block of $('script[type="application/ld+json"]').toArray()) {
    try {
      ...
      for (const node of nodes) {
        ...
        if (!types.includes('WebPage') || node?.url !== link) continue;
        const date = publicationDate(node.datePublished);
        if (date != null) return date;
      }
    } catch {
      // Other JSON-LD blocks can remain usable when one is malformed.
    }
  }
  return undefined;
}
...
async function fetchDetails(listing, browser) {
  try {
    ...
    return {
      ...
      publishedAt: readPublicationDate($, listing.link) ?? listing.publishedAt,
...
const config = {
  ...
  // imaxx offers no price filter on its search at all, so no URL of theirs can carry one.
