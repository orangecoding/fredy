...
async function doGeocode(address) {
  ...
  const url = `${API_URL}?q=${encodeURIComponent(address)}&format=json&countrycodes=ch`;
...
async function doAutocomplete(query) {
  ...
  const url = `${API_URL}?q=${encodeURIComponent(query)}&format=json&addressdetails=1&limit=5&countrycodes=ch`;
