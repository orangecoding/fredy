...
function getDemoUser() {
  return userStorage.getUsers(false).find((user) => user.username === userStorage.DEMO_USERNAME) || null;
}
...
export async function seedDemoJob(providers) {
  ...
  jobStorage.upsertJob({
    ...
    notificationAdapter: [{ id: 'console', fields: {} }],
...
export async function warnOnDefaultAdminPassword() {
  try {
    const admin = userStorage.getUsers(true).find((user) => user.username === userStorage.ADMIN_USERNAME);
