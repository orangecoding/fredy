...
/**
 * The demo job's notification adapter entry.
 *
 * Shaped exactly like one the UI writes (`{ id, name, fields }`) - the adapter table renders the
 * `name` column, so an entry without it showed a nameless row and the visitor could not tell which
 * adapter the demo uses. The label is read from the adapter's own config rather than hardcoded, so
 * it cannot drift from what the rest of the UI calls it.
 *
 * @returns {Promise<{id: string, name: string, fields: Object}>}
 */
async function consoleAdapterEntry() {
  const adapters = await getNotificationAdapters();
  const consoleAdapter = adapters.find((adapter) => adapter.config?.id === 'console');
  return { id: 'console', name: consoleAdapter?.config?.name ?? 'Console', fields: {} };
}
...
export async function seedDemoJob(providers) {
  ...
  jobStorage.upsertJob({
    ...
    notificationAdapter: [await consoleAdapterEntry()],
 ...
 * demo database does not grow without bound. The demo job itself, its live listings, the demo
 * user and the seeded settings are never touched.
 *
