 ...
 */
async function demoAdapterEntry() {
  const adapters = await getNotificationAdapters();
  const demoAdapter = adapters.find((adapter) => adapter.config?.id === 'demo');
  return { id: 'demo', name: demoAdapter?.config?.name ?? 'Demo', fields: {} };
}
...
export async function seedDemoJob(providers) {
  ...
  jobStorage.upsertJob({
    ...
    notificationAdapter: [await demoAdapterEntry()],
