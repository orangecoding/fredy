...
/** Both networks are Italian, so every agency writing a stamp writes it on an Italian clock. */
const NETWORK_ZONE = 'Europe/Rome';
 ...
 * carry no date anywhere, so this is the one place it can be read from. The string is a bare wall
 * clock in the agency's timezone, which for both networks is Italy; reading it as UTC would date an
 * advert published this afternoon up to two hours into the future.
 *
