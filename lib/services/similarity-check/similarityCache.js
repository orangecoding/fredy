 ...
 * @returns {void}
 */
/** @type {Map<string, {provider: string, jobId: string, title: string}>} hash → source info for debugging */
let cacheSource = new Map();
...
export const initSimilarityCache = () => {
  ...
  for (const entry of allEntries) {
    const hash = toHash(entry?.title, entry?.price, entry?.address);
    newCache.add(hash);
    newSource.set(hash, { provider: entry.provider, jobId: entry.job_id, title: entry.title });
  }
  // Atomic swap to avoid mutating the cache while it may be iterated elsewhere
  cache = newCache;
  cacheSource = newSource;
  logger.debug(`Similarity cache rebuilt: ${cache.size} entries from ${allEntries.length} listings`);
};
 ...
 * @returns {{ duplicate: boolean, source?: { provider: string, jobId: string, title: string } }}
 */
export const checkAndAddEntry = ({ title, address, price }) => {
  const hash = toHash(title, price, address);
  if (cache.has(hash)) {
    return { duplicate: true, source: cacheSource.get(hash) };
  }
  cache.add(hash);
  cacheSource.set(hash, { provider: 'current-run', jobId: '', title });
