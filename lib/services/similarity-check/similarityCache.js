 ...
 * (identified by a tuple of job, title, price and address) has been seen before.
 *
 * Design notes:
 ...
 *
 * Each entry is an SHA-256 hex digest produced by toHash(jobId, title, price, address).
 * @type {Set<string>}
 */
let cache = new Set();
 ...
 *
 * The listing is identified by the combination of its job, title, price and
 * address. This keeps cross-provider deduplication within a job without hiding
 * the same listing from another job. Null/undefined fields are ignored during
 * hashing. Falsy-but-valid values (e.g., price 0) are preserved.
 *
 * @param {Object} params - Listing fields
 * @param {string} params.jobId - Job containing the listing
 ...
 */
export const checkAndAddEntry = ({ jobId, title, address, price }) => {
 ...
 */
export const removeEntry = ({ jobId, title, address, price }) => {
  return cache.delete(toHash(jobId, title, price, address));
};

/**
 * Generate an SHA-256 hash from a listing's similarity fields.
...
function toHash(jobId, title, price, address) {
  const normalizedAddress = typeof address === 'string' ? address.replace(/\s*\([^)]*\)/g, '') : address;
  const normalized = [jobId, title, price, normalizedAddress]
