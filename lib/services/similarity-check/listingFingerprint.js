...
export function parseAddress(address) {
  ...
  // Austria and Switzerland write four digits ("1030 Wien", "8001 Zürich"). Read as one only when
  // the address carries no German five-digit postcode, where a four-digit token is more likely a
  // house number. Without this every Viennese address reduced to the locality {wien}, and two
  // different flats of the same size in different districts matched as the same place.
  const zipPattern = tokens.some((token) => /^\d{5}$/.test(token)) ? /^\d{5}$/ : /^\d{4}$/;

  for (let i = 0; i < tokens.length; i++) {
    const token = tokens[i];
    if (result.zip == null && zipPattern.test(token)) {
      result.zip = token;
      consumed.add(i);
      continue;
    }
...
export function isLikelyDuplicate(a, b) {
  ...
  // Two postcodes both known and different are two places, whatever the locality words share:
  // "1030 Wien" and "1200 Wien" overlap on "wien" alone.
  if (a.zip != null && b.zip != null && a.zip !== b.zip) return false;
