...
/** Written in place of anything that identifies the deployment or its operator. */
const REDACTED = '<redacted>';

/**
 * Setting keys whose value is a credential rather than configuration. Matched case-insensitively
 * against the key name, so a setting added later is covered without anyone remembering this file.
 * @type {RegExp}
 */
const SECRET_KEY_PATTERN = /(secret|token|password|passwd|api[-_]?key|credential|private[-_]?key|salt)/i;

/**
 * Redact a URL-like string so it can safely appear in a bundle attached to a public issue.
 *
 * Everything naming the deployment goes: credentials, the host, and every query parameter value.
 * What triage actually reads stays: the scheme (the session cookie's `Secure` flag hangs off it,
 * see `lib/api/security.js`), the port, the path, and the parameter names. Values that are not
 * URLs, and URLs carrying no host (`/db`, `mailto:`), are returned untouched.
 *
 * @param {string} value
 * @returns {string}
 */
function redactUrlLike(value) {
  ...
  // Without a host there is nothing host-identifying to hide - plain paths land here.
  ...
  return `${url.protocol}//${REDACTED}${port}${url.pathname}${url.search}${url.hash}`;
}

/**
 * Fingerprint the hostname instead of printing it.
 *
 * `os.hostname()` is the machine name on bare metal and the container id under Docker; neither
 * belongs in a file users attach to public issues. A short digest preserves the only property
 * triage needs from it - that two bundles from the same host can be recognised as such.
 *
 * @returns {string}
 */
function redactedHostname() {
  const digest = crypto.createHash('sha256').update(os.hostname()).digest('hex').slice(0, 8);
  return `${REDACTED}:${digest}`;
}

/**
 * Render a single settings value for sys.txt, redacting whatever must not leave the instance.
 *
 * @param {string} key Setting name, used to recognise credentials.
 * @param {any} value Setting value as stored.
 * @returns {string} The line's right-hand side.
 */
function renderSettingValue(key, value) {
...
/**
 * Build a plaintext system / runtime report for inclusion in the debug zip.
 *
 * The bundle is meant to be attached to a public issue, so nothing that identifies the deployment
 * survives serialization: the hostname is reduced to a digest, credential-shaped settings are
 * replaced wholesale, and any setting holding a URL keeps only its scheme, port, path and
 * parameter names - never the host, the credentials, or a parameter value.
 *
...
export async function buildSystemInfo({ settings = null } = {}) {
  ...
  lines.push(`Hostname:          ${redactedHostname()}`);
  ...
  if (settings && typeof settings === 'object') {
    ...
    for (const [key, value] of Object.entries(safe)) {
      lines.push(`${key}: ${renderSettingValue(key, value)}`);
    }
    lines.push('');
  }
