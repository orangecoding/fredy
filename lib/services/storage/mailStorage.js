 ...
 * @param {string} userId
 * @param {number} [limit]
 * @returns {Array<{id:string,subject:string|null,textBody:string|null}>}
 */
export function getUnmatchedMailMessages(userId, limit = 200) {
  return SqliteConnection.query(
    `SELECT m.id, m.subject, m.text_body AS textBody
      ...
      ORDER BY COALESCE(m.received_at, m.created_at) DESC
      LIMIT @limit`,
    { userId, limit: Math.max(1, Math.min(500, Number(limit) || 200)) },
  );
}
 ...
 * @param {'applied'|'rejected'|'accepted'} [assignment.status]
 * @returns {boolean}
 */
export function assignMailMessageToListing({ messageId, listingId, userId, method, confidence, status }) {
  ...
  const allowedStatuses = ['applied', 'rejected', 'accepted'];
  if (!allowedMethods.includes(method)) throw new Error(`Invalid mail match method: ${method}`);
  if (status !== undefined && !allowedStatuses.includes(status)) throw new Error(`Invalid listing status: ${status}`);
  ...
  SqliteConnection.withTransaction((db) => {
    ...
    if (status !== undefined) {
      db.prepare(`UPDATE listings SET status = @status WHERE id = @listingId`).run({
        listingId,
        status: JSON.stringify({ status, setAt: now }),
      });
    }
    assigned = true;
  });
