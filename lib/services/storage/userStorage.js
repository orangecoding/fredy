...
export const getUsers = (withPassword) => {
  const jobs = jobStorage.getJobs().value();
  return db.chain
    .get('user')
    .value()
    .map((user) => ({
...
export const getUser = (id) => {
  const jobs = jobStorage.getJobs().value();
  const user = db.chain.get('user').find((user) => user.id === id);
  if (user == null) {
    return null;
  }
  return {
...
export const upsertUser = ({ username, password, userId, isAdmin }) => {
  const user = db.chain.get('user').filter((u) => u.id !== userId);
  user.push({
    id: userId || nanoid(),
    username,
