/*
 * Copyright (c) 2026 by Christian Kellner.
 * Licensed under Apache-2.0 with Commons Clause and Attribution/Naming Clause
 */

import * as hasher from '../security/hash.js';
import { nanoid } from 'nanoid';
import crypto from 'crypto';
import SqliteConnection from './SqliteConnection.js';
import { getSettings } from './settingsStorage.js';
import { inDevMode } from '../../utils.js';

/**
 * Generate a permanent, non-expiring MCP API token.
 * These tokens are secrets that never expire and are used for MCP authentication.
 * @returns {string}
 */
const generateMcpToken = () => `fredy_${crypto.randomBytes(32).toString('hex')}`;

/** Username of the built-in administrator account. */
export const ADMIN_USERNAME = 'admin';

/**
 * Password the administrator account is created with on a fresh install.
 *
 * Exported so the startup check that warns about an unchanged admin password compares against
 * the very value used to create it, instead of a second copy that could drift.
 */
export const DEFAULT_ADMIN_PASSWORD = 'admin';

/** Username of the visitor account a demo instance hands out. */
export const DEMO_USERNAME = 'demo';

/** Password of the demo account. Deliberately guessable - it only exists in demo mode. */
export const DEMO_PASSWORD = 'demo';

/**
 * Get all users, without credentials.
 *
 * `password` (the hash) and `mcp_token` (a permanent bearer credential) are left out of the
 * projection entirely rather than nulled afterwards, so there is no way to accidentally ask for
 * them: both used to travel to the browser with every user listing, ending up in memory, in
 * devtools and in any support bundle. Callers that genuinely need them use
 * {@link getUserWithSecretsByUsername} or {@link getMcpToken}.
 *
 * numberOfJobs is computed via a subquery for each user.
 *
 * @returns {User[]} Array of users ordered by username.
 */
export const getUsers = () => {
  const rows = SqliteConnection.query(
    `SELECT u.id, u.username, u.last_login AS lastLogin, u.is_admin AS isAdmin,
            (SELECT COUNT(1) FROM jobs j WHERE j.user_id = u.id) AS numberOfJobs
       FROM users u
       ORDER BY u.username`,
  );
  return rows.map((u) => ({ ...u, isAdmin: !!u.isAdmin }));
};

/**
 * Look a user up by name, credentials included. Only for the login flow, which needs the stored
 * hash to verify against and to decide whether it should be upgraded.
 *
 * @param {string} username
 * @returns {Object|null} The raw row, or null when there is no such user.
 */
export const getUserWithSecretsByUsername = (username) => {
  return (
    SqliteConnection.query(
      `SELECT u.id, u.username, u.password, u.is_admin AS isAdmin
         FROM users u
        WHERE u.username = @username
        LIMIT 1`,
      { username },
    )[0] ?? null
  );
};

/**
 * Read a user's MCP API token. Separate from {@link getUser} so handing out a permanent bearer
 * credential is always a deliberate act rather than a side effect of loading a user.
 *
 * @param {string} id - User id.
 * @returns {string|null}
 */
export const getMcpToken = (id) => {
  return (
    SqliteConnection.query(`SELECT mcp_token AS mcpToken FROM users WHERE id = @id LIMIT 1`, { id })[0]?.mcpToken ??
    null
  );
};

/**
 * Get a single user by id, without credentials.
 *
 * @param {string} id - User id (primary key).
 * @returns {User|null} The user when found; otherwise null.
 */
export const getUser = (id) => {
  const rows = SqliteConnection.query(
    `SELECT u.id, u.username, u.last_login AS lastLogin, u.is_admin AS isAdmin,
            (SELECT COUNT(1) FROM jobs j WHERE j.user_id = u.id) AS numberOfJobs
       FROM users u
      WHERE u.id = @id
      LIMIT 1`,
    { id },
  );
  const u = rows[0];
  if (!u) return null;
  return { ...u, isAdmin: !!u.isAdmin };
};

/**
 * Get a single user by name, without credentials. Used by proxy (forward) auth, which only ever
 * needs to know whether the name a trusted proxy vouches for belongs to an existing account.
 *
 * @param {string} username
 * @returns {{id: string, username: string, isAdmin: boolean}|null} The user when found; otherwise null.
 */
export const getUserByUsername = (username) => {
  const user = getUserWithSecretsByUsername(username);
  if (user == null) return null;
  return { id: user.id, username: user.username, isAdmin: !!user.isAdmin };
};

/**
 * Insert a new user or update an existing one.
 *
 * Behavior:
 * - When userId is provided and exists: updates username and isAdmin. Password is only updated when a non-empty password is provided.
 * - When userId is missing or does not exist: inserts a new user with a freshly generated id. last_login is initialized to null.
 * - Passwords are hashed using the same hashing function used for login comparison.
 *
 * @param {Object} params
 * @param {string} params.username - Username (must be unique in DB).
 * @param {string} [params.password] - Plain text password to set; if omitted on update, existing hash is preserved.
 * @param {string} [params.userId] - Existing user id to update; if missing, a new id is generated.
 * @param {boolean} params.isAdmin - Whether the user should have admin privileges.
 * @returns {Promise<void>}
 */
export const upsertUser = async ({ username, password, userId, isAdmin }) => {
  const id = userId || nanoid();
  // Check if user exists
  const exists = SqliteConnection.query(`SELECT 1 FROM users WHERE id = @id LIMIT 1`, { id }).length > 0;
  if (exists) {
    // Update existing user. Update password only if provided (non-empty string)
    if (password && password.length > 0) {
      SqliteConnection.execute(
        `UPDATE users SET username = @username, password = @password, is_admin = @is_admin WHERE id = @id`,
        { id, username, password: await hasher.hash(password), is_admin: isAdmin ? 1 : 0 },
      );
    } else {
      SqliteConnection.execute(`UPDATE users SET username = @username, is_admin = @is_admin WHERE id = @id`, {
        id,
        username,
        is_admin: isAdmin ? 1 : 0,
      });
    }
  } else {
    SqliteConnection.execute(
      `INSERT INTO users (id, username, password, last_login, is_admin, mcp_token)
       VALUES (@id, @username, @password, @last_login, @is_admin, @mcp_token)`,
      {
        id,
        username,
        password: await hasher.hash(password || ''),
        last_login: null,
        is_admin: isAdmin ? 1 : 0,
        mcp_token: generateMcpToken(),
      },
    );
  }
};

/**
 * Update the last_login timestamp to now for the given user.
 *
 * @param {{userId: string}} params - Parameters.
 * @param {string} params.userId - The user's id.
 * @returns {void}
 */
export const setLastLoginToNow = ({ userId }) => {
  SqliteConnection.execute(`UPDATE users SET last_login = @now WHERE id = @id`, { id: userId, now: Date.now() });
};

/**
 * Remove a user by id, together with everything that belongs to them alone.
 *
 * Most of it the database does: jobs, the watch list, notification channels and the OAuth tables
 * all reference `users(id)` with ON DELETE CASCADE, and listings follow their job. Two things do
 * not, and both used to survive the account forever:
 *
 * - `settings` rows carry a `user_id` but no foreign key (see migration 6), so the finance
 *   profile, the home addresses and every other per-user setting stayed behind as rows nothing
 *   could ever read again.
 * - `jobs.shared_with_user` is a JSON array, which no foreign key can reach into. A deleted user's
 *   id lingered in the share list of every job they had been given access to.
 *
 * All of it in one transaction: a half-removed user is worse than either end state.
 *
 * @param {string} userId - The id of the user to remove.
 * @returns {void}
 */
export const removeUser = (userId) => {
  SqliteConnection.withTransaction((db) => {
    db.prepare(`DELETE FROM settings WHERE user_id = @id`).run({ id: userId });
    // json_group_array over the filtered rows rebuilds the list without this id; an empty result
    // is '[]', which is exactly what an unshared job holds.
    db.prepare(
      `UPDATE jobs
          SET shared_with_user = (SELECT json_group_array(value)
                                    FROM json_each(jobs.shared_with_user)
                                   WHERE value <> @id)
        WHERE EXISTS (SELECT 1 FROM json_each(jobs.shared_with_user) WHERE value = @id)`,
    ).run({ id: userId });
    db.prepare(`DELETE FROM users WHERE id = @id`).run({ id: userId });
  });
};

/**
 * Update just the stored password hash for a given user.
 * Used to transparently re-hash legacy (unsalted SHA-256) password digests
 * after a successful login, without requiring the user to change their password.
 *
 * @param {{userId: string, passwordHash: string}} params
 * @returns {void}
 */
export const updatePasswordHash = ({ userId, passwordHash }) => {
  SqliteConnection.execute(`UPDATE users SET password = @password WHERE id = @id`, {
    id: userId,
    password: passwordHash,
  });
};

/**
 * Ensure the demo user matches the demo mode setting.
 *
 * Behavior:
 * - When config.demoMode is false: remove the demo user (and its cascading data via FKs).
 * - When config.demoMode is true: ensure a 'demo' user exists with password 'demo' and NO admin
 *   rights, demoting it when an older Fredy created it as an admin. The demo guards throughout the
 *   API are written as `demoMode && !isAdmin(request)`, so an admin demo user bypasses all of them -
 *   including the protection of the demo job itself.
 *
 * Security: The demo user's password is set to a known value ('demo') and should only be enabled in demoMode.
 * @returns {Promise<void>}
 */
export const ensureDemoUserExists = async () => {
  const settings = await getSettings();
  if (!settings.demoMode) {
    if (!inDevMode()) {
      // Through removeUser rather than a DELETE of its own: the demo account collects settings,
      // jobs and shares like any other, and a second deletion path is a second place to forget
      // what a user leaves behind.
      const demo = SqliteConnection.query(`SELECT id FROM users WHERE username = @username`, {
        username: DEMO_USERNAME,
      });
      for (const user of demo) {
        removeUser(user.id);
      }
    }
    return;
  }
  // Ensure demo user exists when demo mode is on
  const existing = SqliteConnection.query(`SELECT id FROM users WHERE username = @username LIMIT 1`, {
    username: DEMO_USERNAME,
  });
  if (existing.length === 0) {
    SqliteConnection.execute(
      `INSERT INTO users (id, username, password, last_login, is_admin, mcp_token)
       VALUES (@id, @username, @password, NULL, 0, @mcp_token)`,
      {
        id: nanoid(),
        username: DEMO_USERNAME,
        password: await hasher.hash(DEMO_PASSWORD),
        mcp_token: generateMcpToken(),
      },
    );
    return;
  }
  SqliteConnection.execute(`UPDATE users SET is_admin = 0 WHERE username = @username AND is_admin <> 0`, {
    username: DEMO_USERNAME,
  });
};

/**
 * Ensure there is at least one administrator in the system.
 *
 * Behavior:
 * - If there are no users at all, create default 'admin' user with password 'admin'.
 * - If users exist but none is admin, promote the first existing user to admin.
 *
 * Security: On a fresh instance, a default admin/admin is created; change this password immediately.
 * @returns {Promise<void>}
 */
/**
 * Validate an MCP API token and return the associated user id.
 * MCP tokens are permanent secrets stored in the users table that never expire.
 * @param {string} token - The raw token string (e.g. fredy_...).
 * @returns {{ userId: string } | null} The user id or null if invalid.
 */
export const validateMcpToken = (token) => {
  if (!token) return null;
  const row = SqliteConnection.query(`SELECT id FROM users WHERE mcp_token = @token LIMIT 1`, { token })[0];
  return row ? { userId: row.id } : null;
};

export const ensureAdminUserExists = async () => {
  const anyUser = SqliteConnection.query(`SELECT id FROM users LIMIT 1`).length > 0;
  if (!anyUser) {
    SqliteConnection.execute(
      `INSERT INTO users (id, username, password, last_login, is_admin, mcp_token)
       VALUES (@id, @username, @password, @last_login, 1, @mcp_token)`,
      {
        id: nanoid(),
        username: ADMIN_USERNAME,
        password: await hasher.hash(DEFAULT_ADMIN_PASSWORD),
        last_login: Date.now(),
        mcp_token: generateMcpToken(),
      },
    );
    return;
  }
  const adminCount = SqliteConnection.query(`SELECT COUNT(1) AS c FROM users WHERE is_admin = 1`)[0]?.c ?? 0;
  if (adminCount === 0) {
    const firstUser = SqliteConnection.query(`SELECT id FROM users LIMIT 1`)[0];
    if (firstUser) {
      SqliteConnection.execute(`UPDATE users SET is_admin = 1 WHERE id = @id`, { id: firstUser.id });
    }
  }
};
