...
export async function runMigrations() {
  ...
  for (const m of files) {
    ...
    try {
      ...
      // Rethrow rather than setting process.exitCode and returning: an exit code only takes
      // effect once the event loop drains, which never happens here because the caller goes on
      // to start the HTTP server and the schedulers. Fredy would serve traffic against a schema
      // missing this migration and everything after it, surfacing later as opaque
      // "no such column" errors far from the cause.
      throw new Error(`Migration failed and was rolled back: ${m.name}`, { cause: e });
    }
  }
...
if (isDirectRun) {
  // `yarn migratedb` must exit non-zero on a failed migration so a deploy pipeline notices.
  try {
    await runMigrations();
  } catch (e) {
