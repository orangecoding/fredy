...
/**
 * Normalize the configured distance-check addresses to an array.
 *
 ...
 *
 * Returns *everything* stored globally, secrets included, and is therefore only safe for
 * server-side callers. Anything that serializes settings into an HTTP response must use
 * {@link getPublicSettings} instead.
 * @returns {Record<string, any>}
 */
...
/**
 * Setting names that must never leave the process.
 *
 * The settings table is a flat key/value store that gets serialized wholesale by the settings
 * endpoint, so a secret stored in it is one forgotten filter away from being handed to every
 * logged-in user. Filtering at the source means a new secret only has to be listed here once,
 * rather than remembered at every call site that returns settings.
 * @type {Set<string>}
 */
const NON_SERIALIZABLE_SETTINGS = new Set(['session_secret']);

/**
 * Global settings with every secret removed - the only variant safe to put in an HTTP response.
 *
 * Note this still contains admin-level configuration (proxy URL, database path, port). Callers
 * serving non-admins have to narrow it further; see the general settings route.
