...
import { computeBenchmark, pricePerSqm } from '../listings/marketBenchmark.js';
import { SCAM_OVERRIDES, detectScamSignals } from '../listings/scamSignals.js';
...
const parseListingStatus = (row) => {
  ...
  if (typeof row.scam_signals === 'string') {
    row.scam_signals = fromJson(row.scam_signals, null);
  }
  return row;
};
 ...
 *
 * @param {string[]} jobIds
 * @returns {{ numberOfListings: number, numberOfActiveListings: number, medianPriceOfListings: number,
 *   medianPriceSampleSize: number, medianPricePerSqm: ({dealType: ('rent'|'buy'), value: number,
...
export const getListingsKpisForJobIds = (jobIds = []) => {
  ...
  return {
    // Every listing ever found, inactive ones included, which is what the dashboard's "of N ever
    // found" names. `numberOfFoundListings` on a job counts only the active ones.
    numberOfListings: totalCount,
    numberOfActiveListings: activeCount,
    medianPriceOfListings: medianPrice,
    // The population the median price is taken over, which is neither the active count nor the
    // price-per-m² sample of one deal type.
    medianPriceSampleSize: pricedCount,
    medianPricePerSqm: medianPricePerSqmForJobIds(jobIds),
  };
};

/**
 * The median price per square metre across a set of jobs, for one deal type only.
 *
 * One number, not two. A rental at 12 EUR/m² and a purchase at 4.800 EUR/m² share a column at
 * magnitudes three orders apart, and a median over both is a figure that describes neither. So the
 * deal type carrying the most measured listings wins, and it travels with the number so the card
 * can say which one it is reporting.
 *
 * The same offset trick the median price above uses: two counted rows and one short read, rather
 * than pulling every listing into memory to sort it there.
 *
 * @param {string[]} jobIds
 * @returns {{dealType: ('rent'|'buy'), value: number, sampleSize: number}|null} `null` when nothing
 *   has a price per square metre yet, which is every instance until the first run after the
 *   migration.
 */
function medianPricePerSqmForJobIds(jobIds) {
  const placeholders = jobIds.map(() => '?').join(',');
  const from = `FROM listings l
                       JOIN jobs j ON j.id = l.job_id
                WHERE l.job_id IN (${placeholders})
                  AND l.manually_deleted = 0
                  AND l.price_per_sqm IS NOT NULL
                  AND j.deal_type = ?`;

  let best = null;
  for (const dealType of [DEAL_TYPES.RENT, DEAL_TYPES.BUY]) {
    const params = [...jobIds, dealType];
    const count = Number(SqliteConnection.query(`SELECT COUNT(1) AS cnt ${from}`, params)?.[0]?.cnt ?? 0);
    if (count === 0 || (best != null && count <= best.sampleSize)) {
      continue;
    }
    const isEven = count % 2 === 0;
    const middle = SqliteConnection.query(
      `SELECT l.price_per_sqm AS value ${from} ORDER BY l.price_per_sqm LIMIT ${isEven ? 2 : 1} OFFSET ${Math.floor((count - 1) / 2)}`,
      params,
    );
    if (middle.length === 0) {
      continue;
    }
    const value = middle.reduce((sum, row) => sum + Number(row.value), 0) / middle.length;
    best = { dealType, value: Math.round(value * 100) / 100, sampleSize: count };
  }
  return best;
}
...
/**
 * The newest active listings across the given jobs.
 *
 * The dashboard's reason to exist is "what came in", and that question was one navigation away.
 * Active only, because a dashboard that leads with listings that are already gone wastes the one
 ...
 */
export const getLatestListingsForJobIds = (jobIds = [], limit = 8) => {
  if (!Array.isArray(jobIds) || jobIds.length === 0) {
    return [];
  }
  // Interpolated rather than bound, like the median query above: SQLite takes no parameter in
  // LIMIT. Bounded to an integer first so nothing but a number can reach the statement.
  const count = Number.isInteger(limit) && limit > 0 && limit <= 50 ? limit : 8;
  const placeholders = jobIds.map(() => '?').join(',');
  return SqliteConnection.query(
    `SELECT id, title, address, price, size, rooms, price_per_sqm, image_url, created_at, job_id
     FROM listings
     WHERE job_id IN (${placeholders})
       AND manually_deleted = 0
       AND is_active = 1
...
/**
 * Per job: how many listings came in on each of the last `days` days, and the total over them.
 *
 * One grouped read rather than one query per job, because the dashboard already knows the job list
 * and the point of the panel is the comparison between them.
 *
 * @param {string[]} jobIds
 * @param {number} [days=7]
 * @param {number} [now=Date.now()]
 * @returns {Record<string, {perDay: number[], total: number}>}
 */
export const getListingActivityPerJob = (jobIds = [], days = 7, now = Date.now()) => {
  if (!Array.isArray(jobIds) || jobIds.length === 0) {
    return {};
  }
  ...
  // The empty calendar first, so a job that found nothing still yields a full series instead of
  // being absent from the result and rendering as a missing row.
  const dayKeys = [];
  for (let offset = span - 1; offset >= 0; offset--) {
    const day = new Date(startOfToday);
    day.setDate(day.getDate() - offset);
  ...
  for (const jobId of jobIds) {
    activity[jobId] = { perDay: new Array(span).fill(0), total: 0 };
  }
  ...
  for (const row of rows) {
    const entry = activity[row.job_id];
    if (entry == null) {
      continue;
    }
    const index = dayKeys.indexOf(toDayKey(new Date(Number(row.created_at))));
    if (index >= 0) {
      entry.perDay[index] += 1;
      entry.total += 1;
    }
  }
 ...
 * stays in the database forever. Deletion is irreversible, hence the guards - a listing is only
 * touched once `retentionDays` have passed since `inactive_since`, and never if somebody has shown
 * they care about it. Two things count as that, and they are the same idea twice: the listing is on
 * a watch list, or somebody has attached documents to it. Uploading the exposé is the point of
 * keeping a listing whose ad is gone, so deleting it on a timer would delete exactly the records
 * the upload feature exists to preserve. `is_active IS NULL` means "never determined" and is left
 * alone, same as {@link deleteInactiveListingsByJobId}.
 *
...
export const purgeExpiredInactiveListings = ({ retentionDays, now = Date.now() }) => {
  ...
  const where = `WHERE is_active = 0
       ...
       AND id NOT IN (SELECT listing_id FROM listing_attachments)`;
 ...
 * saves a second read per listing. Title, address, size and rooms come along because a price change
 * notification renders the listing from this row.
 *
 * @param {Object} [params]
 ...
 * @param {number} [params.now=Date.now()] Injectable for tests.
 * @returns {Array<{id: string, link: string, provider: string, price: number|null, job_id: string, title: string|null, address: string|null, size: number|null, rooms: number|null}>}
 */
export const getListingsDueForPriceCheck = ({
  ...
  return SqliteConnection.query(
    `SELECT id, link, provider, price, job_id, title, address, size, rooms
     FROM listings
     WHERE (is_active IS NULL OR is_active = 1)
       AND manually_deleted = 0
...
export const storeListings = (jobId, providerId, listings) => {
  ...
  SqliteConnection.withTransaction((db) => {
    ...
    //
    // The conflict is no longer a no-op, because the portal's own date can arrive on a later run
    // than the row did: a stored listing takes the date when this scrape carries one and the row
    // has none, and keeps the one it has otherwise. A statement that updates also returns, so
    // `RETURNING id` now answers for the conflicting row as well and the lookup below is left as
    // the fallback for the case where nothing came back at all.
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, build_year, energy_class, title,
                             ...
                             published_at, price_per_sqm)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @rooms, @build_year, @energy_class, @title, @image_url,
               @description, @address, @link, @created_at, 1, @latitude, @longitude, @published_at, @price_per_sqm)
       ON CONFLICT(job_id, hash) DO UPDATE SET published_at = COALESCE(@published_at, listings.published_at)
       RETURNING id`,
    );
    ...
    for (const item of listings) {
      const params = {
        ...
        published_at: Number.isFinite(item.publishedAt) ? item.publishedAt : null,
        // Intrinsic to the row, so it is written with it rather than in a pass of its own. The
        // comparison against the area needs neighbours and happens afterwards, in
        // `applyMarketBenchmark`.
        price_per_sqm: pricePerSqm(item.price, item.size),
      };
      ...
      // The same reason, for the same kind of step. `detectScamSignals` compares this against the
      // area median to decide whether a listing is suspiciously cheap, and it reads the object it
      // is handed rather than the row. Writing the figure only into the table left that comparison
      // with an undefined on one side, so the signal fired for everything the migration backfilled
      // and for nothing found afterwards.
      item.price_per_sqm = params.price_per_sqm;
    }
  });
...
/**
 * Measure freshly stored listings against what a square metre costs around them.
 *
 * Runs after the listings are in the table, because the median is taken over rows and the listing
 * being measured has to be excludable by id. It also runs after the similarity and area filters
 * have left their tombstones, so a batch never benchmarks a listing that is on its way out.
 *
 * The computed figures are written back onto the passed objects as well as into the table. Nothing
 * downstream in the pipeline reads them today, but a listing that carries its own numbers is the
 * same shape the query returns, and the alternative is a second read to learn what was just
 * written.
 *
 * A listing without coordinates, or one whose job has no deal type, is skipped in silence. There is
 * nothing to compare it against, and an absent benchmark is what the UI already renders for
 * everything the sample is too thin for.
 *
 * @param {string} jobId Job the listings belong to.
 * @param {Array<{id: string, latitude?: number|null, longitude?: number|null}>} listings
 * @returns {void}
 */
export const applyMarketBenchmark = (jobId, listings) => {
  if (!jobId || !Array.isArray(listings) || listings.length === 0) {
    return;
  }
  const dealType = SqliteConnection.query(`SELECT deal_type AS dealType FROM jobs WHERE id = ?`, [jobId])?.[0]
    ?.dealType;
  if (dealType == null) {
    return;
  }

  const runQuery = (sql, params) => SqliteConnection.query(sql, params);
  for (const listing of listings) {
    const benchmark = computeBenchmark(listing, dealType, runQuery);
    if (benchmark == null) {
      continue;
    }
    ...
    listing.market_median_sqm = benchmark.medianPricePerSqm;
    listing.market_sample_size = benchmark.sampleSize;
    listing.market_radius_km = benchmark.radiusKm;
  }
};

/**
 * Read freshly stored listings for the marks of a rental scam.
 *
 * Runs after {@link applyMarketBenchmark}, because one of the signals is the listing being far under
 * what a square metre costs around it, and that comparison does not exist until the benchmark is
 * written. The rest is read off the row.
 *
 * Never touches `scam_override`. That column holds what the user said, and a job run finding nothing
 * this time must not quietly withdraw a warning somebody put there by hand.
 *
...
export const applyScamSignals = (listings) => {
  ...
  for (const listing of listings) {
    ...
    const signals = detectScamSignals(listing);
    const payload = signals.length === 0 ? null : JSON.stringify(signals);
    SqliteConnection.execute(`UPDATE listings SET scam_signals = @signals WHERE id = @id`, {
      id: listing.id,
      signals: payload,
    });
    listing.scam_signals = signals.length === 0 ? null : signals;
  }
};

/**
 * Record what the user decided about a listing, or clear that decision.
 *
 * A plain column rather than the JSON payload the status column uses: there is one value and no
 * second thing worth knowing about it. When the warning came off, or went on, is not something any
 * screen asks.
 *
 * @param {string} id
 * @param {('scam'|'safe'|null)} override `null` hands the listing back to the detector.
 * @returns {number} Rows affected, 0 when there is no such listing.
 */
export const setListingScamOverride = (id, override) => {
  if (!id) return 0;
  const normalized = override == null ? null : String(override).toLowerCase();
  if (normalized != null && !Object.values(SCAM_OVERRIDES).includes(normalized)) {
    throw new Error(`Invalid scam override: ${override}`);
  }
  const res = SqliteConnection.execute(`UPDATE listings SET scam_override = @override WHERE id = @id`, {
    id,
    override: normalized,
  });
...
/**
 * Build the WHERE clause and bind parameters every listings query shares.
 *
 * Extracted from {@link queryListings} because the bulk delete has to remove exactly the rows the
 * overview is showing. Two copies of these clauses would be two answers to "what does this filter
 * match", and the one that disagreed would be the one deleting rows.
 *
 * The clause is written against the aliases `l` (listings), `j` (jobs) and `wl` (watch_list), so
 * every caller has to join all three - see {@link LISTING_FILTER_FROM_SQL}.
 *
 * `limit` and `offset` are deliberately not seeded here: paging belongs to the query that pages.
 *
 * @param {Object} filters - The same filter fields {@link queryListings} takes.
 * @returns {{whereSql: string, params: Object}} The `WHERE ...` text (empty string when nothing
 *   filters) and its named bind parameters.
 */
function buildListingWhere({
  ...
  return { whereSql: whereParts.length ? `WHERE ${whereParts.join(' AND ')}` : '', params };
}

/**
 * The three tables {@link buildListingWhere} writes its clause against.
 *
 * `watch_list` is joined even where no watched flag is selected, because the watchlist filter
 * references `wl` and a query that left it out would fail only for the users who use that filter.
 * @type {string}
 */
const LISTING_FILTER_FROM_SQL = `FROM listings l
         LEFT JOIN jobs j ON j.id = l.job_id
 ...
 * @param {string|null} [params.sortField=null] - One of: 'created_at','published_at','price','size','provider','title'.
...
export const queryListings = ({
  ...
  const { whereSql: whereSqlWithAlias, params: filterParams } = buildListingWhere({
    activityFilter,
    jobNameFilter,
    jobIdFilter,
    providerFilter,
    ...
    travelTimeFilter,
    connectivityMinDown,
    connectivityFiberOnly,
    connectivityMobileMask,
    userId,
    isAdmin,
    hiddenOnly,
  });
  const params = { ...filterParams, limit: safePageSize, offset };
  ...
  // Ordered by the date the portal itself states - a flat published two weeks ago and only
  // discovered today reads as the older of the two. Listings whose portal says nothing fall back
  // to the moment Fredy found them, which is what created_at has always meant.
  const orderSqlWithAlias = safeSortExpr
  ...
  const rows = SqliteConnection.query(
    `SELECT l.*,
            ...
            -- A scalar subquery rather than another LEFT JOIN: joining the attachments would
            -- return one row per document and multiply the page. Covered by
            -- idx_listing_attachments_listing, so it costs an index seek per row.
            (SELECT COUNT(*) FROM listing_attachments la WHERE la.listing_id = l.id) AS attachmentCount
     ${LISTING_FILTER_FROM_SQL}
       ${whereSqlWithAlias}
         ${orderSqlWithAlias}
...
/**
 * Delete every listing matching a filter, rather than a list of ids.
 *
 * This is what the overview's bulk delete runs. It takes the same filter object
 * {@link queryListings} takes and removes exactly the rows that query would return - not the page,
 * the whole set - which is why both go through {@link buildListingWhere}. Sending the ids instead
 * would mean paging them out of the database and back in, and would race every listing the
 * scheduler stored in between.
 *
 * There is no separate access check here, and none is needed: the non-admin scoping clause is part
 * of the shared WHERE, so a filter can only ever name rows the user is allowed to see. Compare
 * {@link deleteListingsById}, which has to validate ids precisely because they come from the body.
 *
 * @param {Object} filters - The same filter fields {@link queryListings} takes, including `userId`
 *   and `isAdmin`. `hiddenOnly` decides whether this acts on the visible rows or the hidden ones.
 * @param {boolean} [hardDelete=false] - Remove the rows outright instead of marking them hidden.
 * @returns {{changes: number}} How many rows were deleted.
 */
export const deleteListingsByFilter = (filters = {}, hardDelete = false) => {
  const { whereSql, params } = buildListingWhere(filters);
  const idSubselect = `SELECT l.id
     ${LISTING_FILTER_FROM_SQL}
     ${whereSql}`;
 ...
 *
 * The link rides along with it, because a provider that serves several countries cannot answer the
 * question on its own: idealista's three sites are one provider, and the hostname a stored row's
 * link carries is what says which of their markets the address is in.
 *
 * @returns {Object[]} Array of listing objects {id, address, provider, link}.
 */
export const getListingsToGeocode = () => {
  return SqliteConnection.query(
    `SELECT id, address, provider, link
     FROM listings
     WHERE is_active = 1
       AND manually_deleted = 0
 ...
 * The link comes with them for the same reason the geocoding sweep takes it: a provider serving
 * several countries cannot say which register a row belongs to, and its link can.
 *
 * @returns {Object[]} Array of {id, latitude, longitude, provider, link}.
 */
export const getListingsToEnrichConnectivity = ({ limit, maxAgeDays, now = Date.now() }) => {
  ...
  return SqliteConnection.query(
    `SELECT id, latitude, longitude, provider, link
     FROM listings
     WHERE is_active = 1
       AND manually_deleted = 0
 ...
 * @param {Object} [options]
 * @param {boolean} [options.includeGeometry=true] - Whether to decode the stored driving route.
 *   Defaults on, because the detail view is what this function was written for. Callers that only
 *   need the row to decide whether the user may touch the listing - the MCP write tools - turn it
 *   off: they pay for the decoding per address and then draw nothing with it.
 * @returns {Object|null}
 */
export const getListingById = (id, userId = null, isAdmin = false, { includeGeometry = true } = {}) => {
  ...
  return row == null ? null : attachTravelTimes([row], { includeGeometry })[0];
};

/**
 * The columns a manual detail fetch is allowed to write.
 *
 * An allow-list rather than "whatever the caller passed", because the value reaching this function
 * comes from a provider's detail page and a provider object carries far more than these five. A
 * typo or a provider growing a new field must not be able to reach a column nobody meant to expose.
 *
 * @type {string[]}
 */
const REFETCHABLE_DETAIL_COLUMNS = ['description', 'address', 'build_year', 'energy_class', 'published_at'];

/**
 * Write whatever a detail page improved onto a listing that already exists.
 *
 * Partial on purpose: `fetchDetails` hands back a whole listing, most of whose fields are copies of
 * what it was given, so the caller works out what actually changed and this writes only that. A
 * blanket update would rewrite every column with its own value and, worse, null the ones the detail
 * page happens not to carry.
 *
 * Deliberately not `setListingAddress`, even when an address is among the fields: that one marks
 * the address as the user's own, clears the stored distances and travel times and deletes the
 * listing's transit rows. A portal-sourced address is not a manual one and must not claim to be.
 *
 * Nothing here touches `price`, so no price-history entry is created.
 *
 * @param {string} id - The listing ID.
 * @param {Object} fields - Keyed by column name; only {@link REFETCHABLE_DETAIL_COLUMNS} are read.
 * @returns {number} Number of rows affected (0 if nothing to write or listing not found).
 */
export const updateListingDetails = (id, fields) => {
  if (!id || fields == null) return 0;

  const columns = REFETCHABLE_DETAIL_COLUMNS.filter((column) => fields[column] != null);
  if (columns.length === 0) return 0;

  const assignments = columns.map((column) => `${column} = @${column}`).join(', ');
  const params = { id };
  for (const column of columns) params[column] = fields[column];

  const res = SqliteConnection.execute(`UPDATE listings SET ${assignments} WHERE id = @id`, params);
  return res?.changes ?? 0;
};
