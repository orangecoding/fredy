...
/**
 * Hard delete listings by provider.
 * This removes listings completely so they can be re-scraped as new.
 * Use with caution - this is irreversible.
 *
 * @param {string} providerId - The provider identifier (e.g., 'flatfox').
 * @returns {{ changes: number }} Number of deleted rows.
 */
export const hardDeleteListingsByProvider = (providerId) => {
  if (!providerId) return { changes: 0 };
  return SqliteConnection.execute(`DELETE FROM listings WHERE provider = @providerId`, { providerId });
};
