 ...
 *
 * @param {string} selectSql - Query selecting `title, address, price` of the rows to delete.
...
function hardDeleteAndEvict(selectSql, deleteSql, params) {
  ...
  removed.forEach((row) =>
    removeEntry({ jobId: row.job_id, title: row.title, address: row.address, price: row.price }),
  );
  return result;
}
...
export const deleteListingsById = (ids, hardDelete = false) => {
  ...
  if (hardDelete) {
    return hardDeleteAndEvict(
      `SELECT job_id, title, address, price
       FROM listings
       WHERE id IN (${placeholders})`,
      `DELETE FROM listings
       WHERE id IN (${placeholders})`,
