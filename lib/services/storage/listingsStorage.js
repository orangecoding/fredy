  ...
  return SqliteConnection.query(
    `SELECT id, link, provider
     ...
     WHERE (is_active IS NULL OR is_active = 1)
       AND manually_deleted = 0
       AND (last_checked_at IS NULL OR last_checked_at <= @staleBefore)
...
/**
 * Record that these listings were probed, whatever the outcome.
 *
 * Called for every probed listing - including the ones that are still online and the ones whose
 * probe errored - otherwise the same listings would come back as due on every single run.
 *
 * @param {string[]} ids
