...
import { USER_LISTING_POINT_SCOPE_SQL, USER_LISTING_SET_SCOPE_SQL } from './listingAccessSql.js';
import { LISTING_STATUSES, LISTING_STATUS_FILTERS } from './listingStatus.js';
 ...
 *
 * Nothing in Fredy sets `is_active` back to 1 - a re-discovered listing conflicts on
 * (job_id, hash) and is left alone - so `inactive_since` never has to be cleared. Should a
 * reactivation path ever be added, it has to clear this column.
 *
...
export const purgeExpiredInactiveListings = ({ retentionDays, now = Date.now() }) => {
  ...
  const result = hardDeleteAndEvict(
    `SELECT job_id, title, address, price, manually_deleted
     FROM listings
     ${where}`,
 ...
 * @param {('applied'|'invited'|'visited'|'documents_sent'|'accepted'|'rejected'|'not_invited'|'none')} [params.statusFilter] - Filter by listing status. 'none' matches NULL.
...
export const queryListings = ({
  ...
  } else if (typeof statusFilter === 'string' && LISTING_STATUS_FILTERS.includes(statusFilter.toLowerCase())) {
...
function hardDeleteAndEvict(selectSql, deleteSql, params) {
  ...
  removed
    ...
    .forEach((row) => removeEntry({ jobId: row.job_id, title: row.title, address: row.address, price: row.price }));
  return result;
}
...
/**
 * Return all listings with the job and similarity fields.
 * This is the single helper requested for simple consumers.
 *
 * @returns {{job_id: string, title: string|null, address: string|null, price: number|null}[]}
 */
export const getAllEntriesFromListings = () => {
  return SqliteConnection.query(`SELECT job_id, title, address, price FROM listings WHERE manually_deleted = 0`);
};
    ...
    if (rows.length === 0) {
      ...
      const upsert = db.prepare(
        `INSERT INTO listing_travel_times (listing_id, label, origin_lat, origin_lng, transit_minutes,
                                           ...
                                           via_stops, estimate_mode, is_estimate, reference_time, computed_at)
         VALUES (@listingId, @label, @originLat, @originLng, @transitMinutes, @transitTransfers, @transitLegs,
                 ...
                 @walkGeometry, @viaStops, @estimateMode, @isEstimate, @referenceTime, @computedAt)
 ...
 *
 * @param {string} id - The listing ID.
 * @param {('applied'|'invited'|'visited'|'documents_sent'|'accepted'|'rejected'|'not_invited'|null)} status - New status, or null to clear.
 * @returns {number} Number of rows affected (0 if listing not found).
 */
export const setListingStatus = (id, status) => {
  if (!id) return 0;
  const normalized = status == null ? null : String(status).toLowerCase();
  if (normalized != null && !LISTING_STATUSES.includes(normalized)) {
