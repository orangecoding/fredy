...
export const queryListings = ({
  ...
  if (filterByJobSettings === true) {
    whereParts.push(`(
        (json_extract(j.spec_filter, '$.minRooms') IS NULL OR l.rooms IS NULL OR l.rooms >= json_extract(j.spec_filter, '$.minRooms')) AND
        (json_extract(j.spec_filter, '$.minSize') IS NULL OR l.size IS NULL OR l.size >= json_extract(j.spec_filter, '$.minSize')) AND
        (json_extract(j.spec_filter, '$.maxPrice') IS NULL OR l.price IS NULL OR l.price <= json_extract(j.spec_filter, '$.maxPrice'))
      )`);
  }
