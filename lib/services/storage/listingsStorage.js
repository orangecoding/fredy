  ...
  SqliteConnection.withTransaction((db) => {
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, title, image_url, description, address,
           link, detailedinfo, additionalpictures, created_at, is_active)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @title, @image_url, @description, @address, @link,
         @detailedinfo, @additionalpictures, @created_at, 1)
    ...
    for (const item of listings) {
      const params = {
        ...
        // include detailed info if available, otherwise null
        // better-sqlite3 only accepts numbers, strings, bigints, buffers and null as bind parameters
        detailedinfo:
          item.detailedinfo != null
            ? typeof item.detailedinfo === 'string'
              ? item.detailedinfo
              : JSON.stringify(item.detailedinfo)
            : null,
        // additionalpictures: store as JSON string when array/object, or null
        // only use lowercase `additionalpictures` from now on
        additionalpictures:
          item.additionalpictures != null
            ? typeof item.additionalpictures === 'string'
              ? item.additionalpictures
              : JSON.stringify(item.additionalpictures)
            : null,
...
export const queryListings = ({
  ...
  // Try to parse JSON columns for detailedinfo and additionalpictures so callers get proper types
  const parsed = rows.map((r) => {
    const copy = { ...r };
    try {
      if (copy.detailedinfo && typeof copy.detailedinfo === 'string') {
        copy.detailedinfo = JSON.parse(copy.detailedinfo);
      }
    } catch {
      // leave as string if parsing fails
    }
    try {
      if (copy.additionalpictures && typeof copy.additionalpictures === 'string') {
        // parse JSON stored in the DB and keep the lowercase field only
        copy.additionalpictures = JSON.parse(copy.additionalpictures);
      }
    } catch {
      // leave as string if parsing fails
    }
  ...
  return { totalNumber, page: safePage, result: parsed };
};
