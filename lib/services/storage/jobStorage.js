...
export const upsertJob = ({
  ...
  if (existing) {
    ...
    SqliteConnection.execute(
      `INSERT INTO jobs (id, user_id, enabled, name, blacklist, provider, notification_adapter, shared_with_user, spatial_filter, spec_filter, deal_type)
       VALUES (@id, @user_id, @enabled, @name, @blacklist, @provider, @notification_adapter, @shareWithUsers, @spatialFilter, @specFilter, @dealType)`,
      {
        id,
        user_id: ownerId,
        enabled: enabled ? 1 : 0,
