...
/**
 * Every channel on the instance, keyed by id.
 *
 * Built once per read call rather than joined per row: the table holds tens of rows on any real
 * instance, so one extra `SELECT` beats a correlated subquery inside the paginated job query, and
 * it keeps the three read paths sharing one code path.
 *
 * @returns {Map<string, import('./configuredAdapterStorage.js').Channel>}
 */
const buildChannelMap = () => new Map(getAllChannels().map((channel) => [channel.id, channel]));

/**
 * Resolve a job's stored channel references into the adapter shape the rest of Fredy expects.
 *
 * This is the single place the indirection is unwrapped. `notify.js`, `FredyPipelineExecutioner`
 * and `Tracker` still receive `{ id, name, fields }` with `id` being the adapter type, exactly as
 * they did when the configuration was inline.
 *
 * A reference to a channel that no longer exists is dropped rather than passed on as a hole, which
 * matches how `notify.js` already tolerates an adapter that has been removed from the codebase.
 *
 * @param {string|null|undefined} raw - The stored JSON column.
 * @param {Map<string, import('./configuredAdapterStorage.js').Channel>} channelMap
 * @returns {Array<{id: string, name: string, fields: Record<string, any>, configuredAdapterId: string}>}
 */
const hydrateNotificationAdapter = (raw, channelMap) => {
  const parsed = fromJson(raw, []);
  // `fromJson` only guards NULL and a JSON parse failure - it returns whatever parsed
  // successfully, array or not. A column holding valid JSON that isn't an array (e.g. '{}',
  // written by something other than upsertJob, or corrupted by hand) would otherwise reach
  // `.map()` and throw, taking down the whole batch read (getJobs/queryJobs hydrate every row
  // in one pass), not just the offending job.
 ...
 * @param {Array<{configuredAdapterId: string}>} params.notificationAdapter - References into
 *   `configured_adapter`. Stored verbatim; the route is responsible for validating them.
 * @param {import('../../utils/commuteBudget.js').CommuteFilter|null} [params.commuteFilter] - How far
 *   this search is willing to travel, and what to do about a listing that is further. Null clears it.
 * @param {string} params.userId - Owner user id for inserts; preserved on updates.
 * @param {'rent'|'buy'} [params.dealType] - Whether this job searches for something to rent or to buy.
 *   Omitted on an update, the stored value is kept; omitted on an insert, it defaults to renting.
 * @returns {void}
 */
    ...
    SqliteConnection.execute(
      `UPDATE jobs
       SET enabled = @enabled,
           ...
           -- Keeping the stored type when none is passed: an update that does not mention the
           -- deal type must not silently reclassify the job.
       ...
       WHERE id = @id`,
      {
        ...
        commuteFilter: commuteFilter ? toJson(commuteFilter) : null,
        dealType: dealType ?? null,
      },
    );
  } else {
    SqliteConnection.execute(
      `INSERT INTO jobs (id, user_id, enabled, name, blacklist, provider, notification_adapter, shared_with_user, spatial_filter, spec_filter, commute_filter, deal_type)
       VALUES (@id, @user_id, @enabled, @name, @blacklist, @provider, @notification_adapter, @shareWithUsers, @spatialFilter, @specFilter, @commuteFilter, @dealType)`,
      {
        id,
        user_id: ownerId,
        enabled: enabled ? 1 : 0,
...
export const getJob = (jobId) => {
  const row = SqliteConnection.query(
    `SELECT j.id,
            ...
            j.commute_filter AS commuteFilter,
  ...
  return {
    ...
    notificationAdapter: hydrateNotificationAdapter(row.notificationAdapter, channelMap),
    spatialFilter: fromJson(row.spatialFilter, null),
    specFilter: fromJson(row.specFilter, null),
    commuteFilter: fromJson(row.commuteFilter, null),
  };
};
