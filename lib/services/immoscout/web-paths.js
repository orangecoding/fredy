 ...
 * @property {string|string[]} realType The mobile API `realestatetype`, or several of them for the
 *   paths that search more than one type at once.
...
/**
 * The defaults a search of this type starts from. A search that names several types carries none:
 * `immobilie-mieten` resolves to `apartmentrent,houserent` with no `exclusioncriteria` at all,
 * where `wohnung-mieten` hides exchange flats.
 *
 * @param {string|string[]} realType The resolved real estate type, or types.
 * @returns {Record<string, string[]>} The defaults, empty when there are none.
 */
export function defaultParamsFor(realType) {
  const realTypes = Array.isArray(realType) ? realType : [realType];
  return (realTypes.length === 1 ? DEFAULT_PARAMS_BY_TYPE[realTypes[0]] : null) ?? {};
}

/**
 * Base web paths: the plain "what am I looking for" slugs that carry no filter.
 *
 * `immobilie-kaufen` and `immobilie-mieten` are the "Alle Immobilien" searches and name two types
 * at once, which the mobile API takes as a comma separated list (#469).
 *
