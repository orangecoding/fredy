...
import { ImmoScoutMobileApiReqParams } from '#types/Immoscount.ts';
...
export function convertWebToMobile(webUrl: string) {
  let url;
  try {
    url = new URL(webUrl);
  } catch {
  ...
  let realType = REAL_ESTATE_TYPE[realTypeKey as keyof typeof REAL_ESTATE_TYPE];
  let additionalParamsFromWebPath;

  if (!realType) {
    // Test for seo optimized apartment path (only used on the ImmoScout web app)
    if (WEB_PATH_TO_APARTMENT_EQUIPMENT_MAP[realTypeKey as keyof typeof WEB_PATH_TO_APARTMENT_EQUIPMENT_MAP]) {
      additionalParamsFromWebPath = WEB_PATH_TO_APARTMENT_EQUIPMENT_MAP[realTypeKey as keyof typeof WEB_PATH_TO_APARTMENT_EQUIPMENT_MAP];
  ...
  const webParams = Object.fromEntries(
    Object.entries(rawParams).filter(([key]) => key !== 'enteredFrom' && PARAM_NAME_MAP[key as keyof typeof PARAM_NAME_MAP]),
  ) as Record<string, string>;

  const geocodes = `/${segments.slice(2, 5).join('/')}`;
  const isRadius = segments.includes('radius');
  const mobileParams: Partial<ImmoScoutMobileApiReqParams> = {
  ...
  for (const [key, val] of Object.entries(webParams)) {
    const _key = PARAM_NAME_MAP[key as keyof typeof PARAM_NAME_MAP] as keyof ImmoScoutMobileApiReqParams;
    if (key === 'equipment' && val) {
      const items = ([] as string[]).concat(val).flatMap((v) => `${v}`.split(','));
      const currentEquipmentParams = mobileParams[_key];
      mobileParams[_key] = [
        ...(currentEquipmentParams ?? []),
        ...items.map((item) => EQUIPMENT_MAP[item.toLowerCase() as keyof typeof EQUIPMENT_MAP]).filter(Boolean),
      ];
    } else {
      mobileParams[_key] = val;
    }
  }
