...
const REAL_ESTATE_TYPE = {
  ...
  'bestandswohnung-mieten': 'apartmentrent',
  'mietwohnungen-mit-tauschwohnungen': 'apartmentrent',
  'wohnung-kaufen': 'apartmentbuy',
  'haus-kaufen': 'housebuy',
  anlageimmobilie: 'investment',
};

const APARTMENT_BASE_SLUGS = {
...
/**
 * House SEO slugs, i.e. the paths the web UI writes instead of a query parameter once a house
 * search carries a single filter. "Haus kaufen" plus the Garage filter becomes
 * `haus-mit-garage-kaufen` rather than `haus-kaufen?equipment=parking`, and the path is the only
 * place the filter shows up, so a slug missing here does not merely lose the filter - the search
 * has no real estate type at all and the job dies on `Real estate type not found`.
 *
 * Both suffixes exist for every slug below; the mapping of each one was read off the
 * `lastSearchApiUrl` the web app reports for that path, so these are the parameters ImmoScout
 * itself resolves the slug to rather than a reading of the German wording.
 */
const HOUSE_BASE_SLUGS = {
  // Category "Haustyp"
...
/**
 * House slugs the web UI only publishes for the buy side. `luxushaus-mieten` answers 410, so the
 * rent counterpart is deliberately not registered.
 */
const HOUSE_BUY_ONLY_BASE_SLUGS = {
  luxushaus: { luxurypromotion: true },
};

/**
 * SEO web path (the last path segment) to the mobile API search it stands for.
 * @type {Record<string, { realType: string, params: Record<string, string[]|boolean> }>}
 */
const WEB_PATH_TO_SEO_SEARCH = {
  // The two paths whose filter does not sit behind the usual "<slug>-kaufen" shape.
  'wohnung-kaufen-mit-balkon': { realType: 'apartmentbuy', params: { equipment: ['balcony'] } },
  'eigentumswohnung-mit-garten': { realType: 'apartmentbuy', params: { equipment: ['garden'] } },
};

/**
 * Registers a table of base slugs under the suffixes the web UI appends to them.
 *
 * @param {Record<string, Record<string, string[]|boolean>>} baseSlugs Base slug to its mobile API parameters.
 * @param {{ buy?: string, rent?: string }} realTypes Real estate type per suffix; a missing one skips that suffix.
 * @returns {void}
 */
function registerSeoSlugs(baseSlugs, { buy, rent }) {
  for (const [slug, params] of Object.entries(baseSlugs)) {
    if (buy) {
      WEB_PATH_TO_SEO_SEARCH[`${slug}-kaufen`] = { realType: buy, params };
    }
    if (rent) {
      WEB_PATH_TO_SEO_SEARCH[`${slug}-mieten`] = { realType: rent, params };
    }
  }
}

registerSeoSlugs(APARTMENT_BASE_SLUGS, { buy: 'apartmentbuy', rent: 'apartmentrent' });
registerSeoSlugs(HOUSE_BASE_SLUGS, { buy: 'housebuy', rent: 'houserent' });
registerSeoSlugs(HOUSE_BUY_ONLY_BASE_SLUGS, { buy: 'housebuy' });
 ...
 * of `?@[\]^_{|}~` can appear in base64, which is what makes the two shape flavours below tellable
 * apart without having to ask ImmoScout which one it sent.
 * @type {RegExp}
 */
 ...
 *
 * ImmoScout hands the shape over in two flavours and the parameter looks the same either way: links
 * from the map editor carry the polyline base64 wrapped, while a shape drawn by hand arrives raw.
 * Decoding a raw one does not fail loudly. `Buffer.from(x, 'base64')` silently drops every
 * character outside the base64 alphabet, and the bytes left over are not valid UTF-8, so the shape
 * reaches the mobile API as a run of U+FFFD (`%EF%BF%BD` once encoded) and it answers
 * `400 Cannot parse shape`. Decoding is therefore only attempted when the input could be base64 at
 * all, and the result is only kept when it decodes to something a polyline could be.
 *
 * @param {string} shape - Raw value of the web URL's `shape` query parameter.
 * @returns {string} The polyline to hand to the mobile API.
 */
function toPolyline(shape) {
...
export function convertWebToMobile(webUrl) {
  ...
  // SEO optimized paths (only used on the ImmoScout web app) carry both the real estate type and
  // the filter the web UI folded into the path.
  const seoSearch = WEB_PATH_TO_SEO_SEARCH[realTypeKey];
  let realType = REAL_ESTATE_TYPE[realTypeKey] ?? seoSearch?.realType;
  let additionalParamsFromWebPath = seoSearch?.params ?? null;
  ...
  const realEstateTypeFromWeb = rawParams.real_estate_type;
  const isBestandswohnung =
    realTypeKey === 'bestandswohnung-mieten' || realEstateTypeFromWeb === 'bestandswohnung-mieten';
  const isTauschwohnungen = realTypeKey === 'mietwohnungen-mit-tauschwohnungen';
  const exclusionCriteria = new Set(
    isBestandswohnung ? ['projectlisting', 'swapflat'] : isTauschwohnungen ? [] : ['swapflat'],
  );
  const mobileParams = {
  ...
  if (exclusionCriteria.size > 0) {
    mobileParams.exclusioncriteria = [...exclusionCriteria];
  }
  ...
  if (isShape && webParams.shape) {
    ...
    mobileParams.shape = toPolyline(browserShape);
  }
  ...
  for (const [key, val] of Object.entries(webParams)) {
    ...
    if (key === 'equipment') {
      ...
      for (const item of items) {
        ...
        if (isTauschwohnungen && normalizedItem === 'swapflat') {
          continue;
        }

        exclusionCriteria.add(normalizedItem);
      }

      mobileParams[PARAM_NAME_MAP[key]] = [...exclusionCriteria];
    } else {
