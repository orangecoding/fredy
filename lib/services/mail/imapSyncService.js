  ...
  if (!metadata.size || metadata.size <= MAX_MESSAGE_BYTES) {
    ...
    if (content?.source) {
      parsed = await simpleParser(content.source, {
        // Most property portals send HTML-only messages. Convert those to plain
        // text while keeping raw HTML out of the browser.
 ...
 */
async function performMailAccountSync(accountId, userId, options) {
  ...
  try {
    ...
    const cursorUid = resume ? Number(account.lastUid) : null;
    const query = resume
      ? { uid: `${cursorUid + 1}:*` }
    ...
    const uids = (Array.isArray(searchResult) ? searchResult : [])
      .map(Number)
      .filter((uid) => Number.isFinite(uid) && (!resume || uid > cursorUid));

    let stored = 0;
    let lastUid = cursorUid;
...
/**
 * Synchronize an account once, sharing an in-flight operation with concurrent
 * manual or scheduled callers. This prevents duplicate IMAP sessions and
 * cursor races without making a second caller fail spuriously.
 *
 * @param {string} accountId
 * @param {string} userId
 * @param {Object} [options]
...
export async function syncMailAccount(accountId, userId, options = {}) {
  ...
  const operation = performMailAccountSync(accountId, userId, options);
  activeSyncs.set(key, operation);
  try {
    return await operation;
  } finally {
    if (activeSyncs.get(key) === operation) activeSyncs.delete(key);
  }
}
