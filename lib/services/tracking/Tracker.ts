...
import { getJobs } from '../storage/jobStorage';
import { getUniqueId } from './uniqueId';
import { config, inDevMode } from '../../utils';
import os from 'os';
import { readFileSync } from 'fs';
import { packageUp } from 'package-up';
import { TrackingObject } from '#types/TrackingObject.ts';
...
export const track = function () {
  ...
  if (config.analyticsEnabled && !inDevMode()) {
    const activeProvider = new Set<string>();
    const activeAdapter = new Set<string>();
 ...
 */
export function trackDemoJobCreated(jobData: Partial<TrackingObject>) {
...
function enrichTrackingObject(trackingObject: Partial<TrackingObject>) {
...
async function getPackageVersion(): Promise<string> {
  try {
    ...
    if (packagePath == null) return 'N/A';
