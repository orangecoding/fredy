...
function stillValid(row, coords, referenceTime, staleBefore, mode, ref = null) {
  ...
  if (ref != null) {
    ...
    // The departure only matters to a timetable. Driving, cycling and walking are measured without
    // one (`placeGroupKey` and the matrix ignore it), so rejecting their rows every time the next
    // working day came round re-measured unchanged answers at the cost of a POI lookup each.
    return (mode !== 'transit' || row.reference_time === referenceTime) && row.computed_at > staleBefore;
  }
...
/**
 * Whether a stored row carries a line the detail map can actually draw.
 *
 * The map falls back to the straight line for a mode it has no geometry for, and says so rather
 * than passing a line through the buildings off as a route. A row that can answer for no mode at
 * all therefore says it on every one of them, which is what a place-type row did for its entire
 * life: the matrix the sweep measures places with answers in durations and nothing else.
 *
 * @param {Object} row
 * @returns {boolean}
 */
function hasDrawableRoute(row) {
  if (row.car_geometry || row.bike_geometry || row.walk_geometry) {
    return true;
  }
  const legs = parseJson(row.transit_legs);
  return Array.isArray(legs) && legs.some((leg) => typeof leg?.geometry === 'string' && leg.geometry.length > 0);
}

/**
 * The real journey to the place a place-type row resolved to.
 *
 * The sweep measures place types with a one-to-many matrix, because "the nearest gym" is a
 * different gym for every listing and asking per candidate is the traffic the Transitous
 * maintainers asked Fredy not to generate. That endpoint answers in durations: no legs, no
 * polylines, nothing to draw. So a place type used to be the one entry that could never show a
 * route, on any mode, however often somebody opened the listing.
 *
 * Which place it is, though, has already been decided and stored. Planning one journey to that
 * fixed point is then the same shape of request a named address makes on the same page - one
 * person, one listing - and it answers for car, bike and walking in the same breath.
 *
 * @param {{latitude: number, longitude: number}} listing
 * @param {Object} row - The stored place-type row.
 * @param {import('../../types/listing.js').Address} address
 * @param {number} referenceTime
 * @returns {Promise<Object|null>} A row for {@link saveListingTravelTimes}, or `null` when there is
 * nothing to add - no place was found, the row is about a different question than the one being
 * asked now, it can already be drawn, or the lookup had no answer.
 */
async function planRouteToPlace(listing, row, address, referenceTime) {
  // -1/-1 is the marker for "we looked and found nothing of that kind nearby". There is no place to
  // plan a journey to, and asking would burn a request to be told so again.
  if (row.origin_lat === -1) {
    return null;
  }
  // A row measured for another category, or in another mode, is about to be re-measured by the
  // sweep, place and all. Routing to last week's gym would be paying for an answer that is already
  // on its way out.
  //
  // The departure it was measured at is deliberately not part of that. It moves on every day the
  // sweep does not run, which is most of them - a row is only re-measured once the listing falls
  // due - and treating it as a mismatch would mean no place type was ever refined except on the day
  // its row was written. The journey is planned for the departure being asked about now, and the
  // row says so.
  if (row.origin_ref !== address.category || (row.estimate_mode ?? 'transit') !== preferredMode(address)) {
    return null;
  }
  if (hasDrawableRoute(row)) {
    return null;
  }

  const result = await getTravelTimes({
    // Listing first, matching the direction the matrix measured in: the question a place type asks
    // is how far it is from this flat to the nearest one of those, not the other way round.
    fromLat: listing.latitude,
    fromLng: listing.longitude,
    toLat: row.origin_lat,
    toLng: row.origin_lng,
    referenceTime,
  });
  if (result.ok !== true) {
    return null;
  ...
  const { times } = result;
  // This journey runs from the listing to the place, so its last leg ends at the plan's END, which
  // the leg normaliser turns into `null` - and the detail view reads a leg without a destination as
  // one that arrives at the listing. Here it arrives at the place.
  const legs = times.transit?.legs;
  const lastLeg = Array.isArray(legs) && legs.length > 0 ? legs[legs.length - 1] : null;
  const transitLegs =
    lastLeg != null && lastLeg.to == null && row.origin_name
      ? [...legs.slice(0, -1), { ...lastLeg, to: row.origin_name }]
      : legs;
  // Every mode falls back to what the row already holds. The plan is the better answer where it has
  // one, but its direct modes stop at a two hour ceiling, and losing a stored walking time to that
  // ceiling would be paying for a route by giving up the number the card leads with.
  return {
    ...carryOver(row, address.label),
    transitMinutes: times.transit?.minutes ?? row.transit_minutes,
    transitTransfers: times.transit?.transfers ?? row.transit_transfers,
    transitLegs: transitLegs ?? parseJson(row.transit_legs),
    carMinutes: times.car?.minutes ?? row.car_minutes,
    carDistanceMeters: times.car?.distanceMeters ?? row.car_distance_meters,
    carGeometry: times.car?.geometry ?? row.car_geometry,
    bikeMinutes: times.bike?.minutes ?? row.bike_minutes,
    bikeGeometry: times.bike?.geometry ?? row.bike_geometry,
    walkMinutes: times.walk?.minutes ?? row.walk_minutes,
    walkGeometry: times.walk?.geometry ?? row.walk_geometry,
    // The departure these times are now about. `computedAt` stays where it was, so the sweep still
    // goes back and asks which gym is nearest on its own schedule rather than having it pushed a
    // month down the road by somebody opening a page.
    referenceTime,
  };
}
 ...
 *
 * Place types take the same path once, to the place they already resolved to: their duration was
 * exact from the start, but the matrix that produced it cannot draw anything. See
 * {@link planRouteToPlace}.
 *
 * @param {{id: string, latitude: number, longitude: number}} listing
 * @param {import('../../types/listing.js').Address[]} addresses
 * @param {Object} [options]
...
export async function refineTravelTimesForListing(listing, addresses, { now = Date.now() } = {}) {
  ...
  const entries = [];
  let routed = 0;
  /** Whether a place type is still waiting for the sweep, which then must not be told it is done. */
  ...
  for (const address of wanted) {
    ...
    // A place type's duration is already exact - the sweep routes to a real place rather than
    // walking in from the nearest stop - so there is no estimate here to improve on. What it has
    // never had is a route: the matrix that measured it answers in durations alone. The place it
    // resolved to is stored, though, so the journey to it can be planned like any other, which is
    // what puts a drawable line on the map instead of the straight one and its apology.
    //
    // Nothing to do at all without a stored row: which place this is, is the sweep's question, and
    // it has not answered it yet.
    if (isPlaceType(address)) {
      // Not this path's question: without a row the sweep has not looked for a place of this kind
      // yet, and a row for another category or mode is about to be re-measured. Stamping the listing
      // below as if it were done took it out of the sweep's queue for a month with that missing.
      if (
        existing == null ||
        existing.origin_ref !== address.category ||
        (existing.estimate_mode ?? 'transit') !== mode
      ) {
        placePending = true;
      }
      if (existing) {
        const planned = await planRouteToPlace(listing, existing, address, referenceTime);
        entries.push(planned ?? carryOver(existing, address.label));
        routed += 1;
      }
      continue;
    }
  ...
  saveListingTravelTimes(listing.id, entries, now, { stamp: !placePending });
  return routed > 0;
}
