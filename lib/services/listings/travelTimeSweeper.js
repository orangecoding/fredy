...
export async function refineTravelTimesForListing(listing, addresses, { now = Date.now() } = {}) {
  ...
  for (const address of wanted) {
    ...
    entries.push({
      ...
      transitMinutes: times?.transit?.minutes ?? null,
      transitTransfers: times?.transit?.transfers ?? null,
      ...
      // An exact journey needs no stop list: it is the journey, not a guess made from one.
      viaStops: null,
      estimateMode: null,
      isEstimate: false,
      referenceTime,
