...
export default class Extractor {
  ...
  /**
   * Parse the page fetched by {@link execute}. The HTML travels with the instance, so two
   * extractors running at the same time cannot read each other's document.
   *
   * @param {string} crawlContainer
   * @param {Record<string, string>} crawlFields
   * @param {string} url Log context only.
   * @returns {Object[]|null}
   */
