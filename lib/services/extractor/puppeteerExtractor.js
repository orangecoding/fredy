      ...
      try {
        ...
        if (captchaResult.detected && options?.twoCaptchaApiKey) {
          ...
          if (solveResult.success && solveResult.cookie) {
            // Extract domain from URL - remove www prefix for cookie domain
            const urlObj = new URL(url);
            const domain = urlObj.hostname.replace(/^www\./, '');
            await applyDataDomeCookie(page, solveResult.cookie, domain);

            // Reload the page with the new cookie - use longer timeout for retry
            logger.debug('Reloading page after applying DataDome cookie...');
            await page.goto(url, {
              waitUntil: options?.waitUntil || 'domcontentloaded',
              timeout: 60_000, // Longer timeout for captcha bypass retry
            });
            await applyPostNavigationHumanSignals(page, preCfg);

            // Debug: Log page state after cookie application
            const reloadedUrl = page.url();
            const reloadedContent = await page.content();
            logger.debug(`After cookie reload - URL: ${reloadedUrl}`);
            logger.debug(`After cookie reload - Content length: ${reloadedContent.length}`);
            if (reloadedContent.includes('captcha-delivery.com')) {
              logger.warn('Still showing captcha after cookie application!');
            }
