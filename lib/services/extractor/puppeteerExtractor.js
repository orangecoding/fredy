...
export default async function execute(url, waitForSelector, options) {
  ...
  try {
    ...
    if (options?.autoScroll) {
      // Scroll incrementally to the bottom so lazy-loaded listings render before we
      // grab the HTML. Each step scrolls one viewport height and waits briefly for
      // the browser to paint new content. Homegate (and similar React SPAs) only
      // mount list items inside the viewport, so without this we only see the first
      // few cards that fit above the fold.
