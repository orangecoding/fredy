...
/** User-Agent required by Transitous usage policy (contact info per their ToS). */
const USER_AGENT = 'fredy-fork/1.0 (forked from https://github.com/orangecoding/fredy; contact@dominikmiskovic.com)';
...
const WALK_SPEED_MS = 5000 / 3600; // 5 km/h in metres per second

/**
 * Estimate walking time from straight-line (haversine) distance.
 * Used as fallback when Transitous does not return a WALK route (distance too large).
 *
 * @param {number} lat1 @param {number} lng1
 * @param {number} lat2 @param {number} lng2
 * @returns {{ duration: number, distance: number, estimated: true }}
 */
function walkingEstimate(lat1, lng1, lat2, lng2) {
  ...
  return { duration: Math.round(distance / WALK_SPEED_MS), distance: Math.round(distance), estimated: true };
}
...
export async function getAllRoutes(startLat, startLng, endLat, endLng) {
  ...
  return {
    [MODES.WALKING]:
      walking.status === 'fulfilled' ? walking.value : walkingEstimate(startLat, startLng, endLat, endLng),
