...
import { getProxyConfig } from '../http/httpClient.js';
      ...
      for (const prov of jobProviders) {
        try {
          ...
          if (!browser && matchedProvider.config.getListings == null) {
            ...
            const launchOptions = proxyConfig ? { proxyUrl: proxyConfig.rawUrl } : {};
            browser = await puppeteerExtractor.launchBrowser(matchedProvider.config.url, launchOptions);
          }
