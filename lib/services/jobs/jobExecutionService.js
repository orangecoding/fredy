 ...
 * @param {Object} deps.settings - Global settings object (read/write). Must include `demoMode`, `interval`, and working-hours attributes used by `duringWorkingHoursOrNotSet`. When `demoMode` is set, execution is restricted to the demo job instead of being disabled entirely.
...
export function initJobExecutionService({ providers, settings, intervalMs }) {
  ...
  async function runAll(respectWorkingHours = true, context = undefined) {
    ...
    const jobs = jobStorage.getJobs().filter((job) => {
      ...
      if (settings.demoMode) return isDemoJob(job.id);
  ...
  async function runSingle(jobId) {
    if (settings.demoMode && !isDemoJob(jobId)) return;
    const job = jobStorage.getJob(jobId);
    if (!job) return;
