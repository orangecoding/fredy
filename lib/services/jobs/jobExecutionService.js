    ...
    try {
      ...
      for (const prov of jobProviders) {
        try {
          ...
          // If the provider supports detail fetching (e.g. immoscout), check user setting
          if (typeof matchedProvider.setFetchDetails === 'function' && job.userId) {
            const userSettings = getUserSettings(job.userId);
            matchedProvider.setFetchDetails(!!userSettings.immoscout_details);
          }
