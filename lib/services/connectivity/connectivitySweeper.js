 ...
 * Enrich one listing.
 *
 * @param {{id: string, latitude: number, longitude: number, provider: string, link?: string}} listing
...
async function enrichListing(listing, settings, now) {
  // Which country *this* listing is in, not which ones its provider serves. The difference is the
  // point: a provider covering several markets - idealista over Spain, Italy and Portugal - would
  // otherwise send every one of its listings to whichever of those countries happens to have a
  // register, which reads the address in the wrong language, spends a throttled request on it and
  // stamps the row "nothing here" for the whole retry interval.
  ...
  // same answer for every Italian listing an instance holds, forever.
  if (source == null) {
...
async function reportUnavailableSources(settings, tally) {
  ...
  // only had Italian listings to stamp would report an outage on the strength of a register left
