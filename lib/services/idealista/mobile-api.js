...
async function mintToken(portal, id) {
  ...
  let response;
  try {
    response = await fetch(`${portal.apiHost}/api/oauth/token?${queryString(query)}`, {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    ...
    const error = new Error(`idealista refused a token: ${response.status} ${refused.slice(0, 200)}`);
...
async function fire(portal, path, query, body, retryOnUnauthorised, method = 'POST') {
  ...
  if (!response.ok) {
    const refused = await response.text().catch(() => '');
    const error = new Error(`idealista api ${path} answered ${response.status}: ${refused.slice(0, 300)}`);
    /** @type {any} */ (error).httpStatus = response.status;
    throw error;
  }
