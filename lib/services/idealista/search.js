...
export async function searchListings(portal, webUrl, jobKey = null) {
  ...
  for (const filters of search.variants) {
    for (let page = 1; page <= pageCap; page++) {
      let answer;
      try {
        answer = await readPage(portal, area, search, filters, page);
        if (!Array.isArray(answer?.elementList)) throw new Error('the search response has no elementList');
      } catch (error) {
        if (adverts.length === 0) throw error;
        logger.warn(`Idealista: stopped at page ${page}; keeping ${adverts.length} adverts (${error.message}).`);
        // An interrupted first run must retry the full catch-up on its next execution.
        return adverts;
      }
      const found = answer.elementList;
