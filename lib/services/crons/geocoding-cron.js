  ...
  try {
    ...
    if (listings.length > 0) {
      for (const listing of listings) {
        ...
        // The listing itself, not just its provider: idealista serves three countries, and the
        // row's own link is what says which of them this address is in. A provider with one
        // country, or a row whose link says nothing, resolves exactly as it did before.
