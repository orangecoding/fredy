...
/**
 * Load an exposé page as markup.
 *
 * Navigating with page.goto on the warmed session ensures real document request headers
 * (Sec-Fetch-Dest: document) that pass DataDome scrutiny, unlike subresource XHR fetches.
 *
 * @param {any} browser the shared browser of the current job run
...
export async function fetchExposeHtml(browser, url) {
  ...
  try {
    // Genuine document navigation via page.goto
    if (typeof page.goto === 'function') {
      ...
      if (status === 200) {
        return typeof page.content === 'function' ? await page.content() : null;
      }

      await reportFailure(status, `exposé ${url}`, `HTTP status ${status}`);
      if (status === 403 || status === 429) {
    ...
    // Fallback for mock/test environments without full page lifecycle
    const result = await page.evaluate(
        ...
        try {
          const response = await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error('immowelt expose request timed out')), timeout),
            ),
          ]);
    ...
    logger.warn(`Could not load immowelt exposé ${url}: ${error?.message || error}`);
    return null;
  }
}
