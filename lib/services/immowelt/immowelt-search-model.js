 ...
 *
 * Only the search area is genuinely reshaped - see {@link parseLocations} for the four spellings
 * immowelt uses for it. One of them, the commute area, cannot be translated here at all: it names
 * a place and a travel time, and the boundary those describe only exists once immowelt's own
 * routing service has drawn it. Those entries are handed back as `commutes` for the transport to
 * resolve against the site - see {@link convertSearchUrlToRequest}.
 *
...
/**
 * Keys that turn a decoded `locations` object into a commute area.
 *
 * `{placeIds: ['STRTDE197842'], duration: '15', mode: 'Walk'}` is "everything within 15 minutes'
 * walk of Schwanseestraße" - a boundary immowelt computes rather than stores, which is why this
 * entry carries no polyline for it.
 */
const ENCODED_COMMUTE_KEYS = new Set(['duration', 'mode']);

/**
 * Travel modes immowelt's routing service knows.
 *
 * Anything else answers 500 rather than a boundary, so an unknown mode is refused here with a
 * message naming it instead of failing mid-run with immowelt's error page. The service itself
 * matches the mode case insensitively; the check follows, and the value reaches it as written.
 */
const COMMUTE_MODES = new Set(['walk', 'bike', 'car', 'transit']);

/**
 * A search area given as "everything within `duration` minutes of `placeId`, travelling by `mode`".
 *
 * @typedef {{placeId: string, duration: number, mode: string}} CommuteArea
 */
 ...
 *
 * Four spellings reach this, and immowelt mixes them freely in one url:
 *
 ...
 *   locations=eyJwbGFjZUlkIjoiQUQwOERF...  → a radius, whose `polyline` is the same boundary again
 *   locations=eyJwbGFjZUlkcyI6WyJTVFJU...  → a commute area: {placeIds, duration, mode}, which
 *                                            carries no boundary at all (see {@link parseCommute})
 *
 ...
 * was saved for. Immowelt's own map drops the ids from its request the moment a drawing exists. A
 * commute area becomes a boundary too once it is resolved, so it drops the ids for the same reason.
 *
 * Several boundaries in one url are unioned rather than intersected: immowelt's own commute search
 * sends one polyline per reachable island (a 15 minute transit area around Munich is 60 of them)
 * and the BFF answers with the sum of what they cover.
 *
 * @param {string|null} raw the raw `locations` parameter
 * @param {string} searchUrl the url, for the error messages
 * @returns {{location: {placeIds?: string[], polylines?: string[]}, commutes: CommuteArea[]}} the
 *   BFF's `criteria.location`, plus the commute areas whose boundary still has to be fetched
...
function parseLocations(raw, searchUrl) {
  ...
  for (const entry of entries) {
    ...
    // A commute area is the one spelling that never carries its boundary: immowelt draws it from
    // the place and the travel time every time the search runs, so the caller has to as well.
  ...
  // The commute areas turn into polylines further down the line, so the places they replace are
  // dropped here already - keeping them would intersect the resolved area with the whole city.
...
/**
 * Read one commute area out of a decoded `locations` entry.
 *
 * `{placeIds: ['STRTDE197842'], duration: '15', mode: 'Walk'}` is what immowelt's "search by
 * commute time" writes into the url. It is the only search area the url describes indirectly: the
 * boundary is drawn by immowelt's routing service from the place's coordinates and the travel
 * time, and nothing in the url is a substitute for it. Searching the place instead would cover the
 * street rather than the quarter around it, so a commute this cannot read stops the job.
 *
 * Only the first place of an entry would reach the routing service - immowelt's own request sends
 * `placeIds[0]` and nothing else - so an entry naming several of them is refused rather than
 * silently searched around one. Two commute areas are spelled as two entries, and those are fine.
 *
 * @param {Record<string, any>} decoded one decoded `locations` entry
 * @param {string} searchUrl the url, for the error messages
 * @returns {CommuteArea} the place, travel time and mode the boundary has to be drawn from
 * @throws {Error} when the entry is not a commute area this translator can hand on
 */
 ...
 *
 * `commutes` is the one part of the search this cannot finish on its own: those areas exist only
 * as a place and a travel time until immowelt's routing service draws them, so they are handed to
 * the transport, which has a session to ask with. A url without one gets an empty list, and the
 * `{criteria, paging}` beside it is already the whole body.
 *
 ...
 * @returns {{criteria: object, paging: {page: number, size: number, order: string}, commutes: CommuteArea[]}}
 *   the POST body for `/serp-bff/search`, plus the search areas still to be resolved into it
...
/**
 * Turn the rings of an isochrone into the polylines the BFF searches by.
 *
 * Immowelt's routing service answers with GeoJSON style coordinates - polygons of rings of
 * `[lng, lat]` - and its own page sends every one of those rings as an entry of
 * `location.polylines`. A transit area is a few dozen disjoint islands, which is why this flattens
 * rather than picking one.
 *
 * @param {number[][][][]} isochrone polygons → rings → `[lng, lat]` points
 * @returns {string[]} one encoded polyline per ring
 */
...
/**
 * Encode a ring the way the BFF's `location.polylines` are encoded.
 *
 * This is Google's polyline algorithm at five decimals, the same one immowelt's map search writes
 * its drawn areas with - verified by encoding a routing answer and getting the exact string
 * immowelt's own page sent for it, character for character.
 *
 * The points arrive as `[lng, lat]` because that is GeoJSON's order and what the routing service
 * answers with; the encoding is latitude first, so the pair is swapped on the way in.
 *
 * @param {number[][]} ring the ring's `[lng, lat]` points
...
/**
 * Encode one delta of a polyline: zig-zag the sign into the low bit, then five bits per character.
 *
 * @param {number} value the difference to the previous coordinate, in units of 1e-5 degrees
 * @returns {string} the characters standing for it
 */
