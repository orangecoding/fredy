...
export const TRACKING_POIS = {
  ...
  LISTING_ATTACHMENT_UPLOAD: 'LISTING_ATTACHMENT_UPLOAD',
  SCAM_MARKED: 'SCAM_MARKED',
  SCAM_DISMISSED: 'SCAM_DISMISSED',
  ...
  /*
   * The MCP server stopped being read-only. Counted apart from the web UI's own NOTES_CREATE and
   * from each other, because the open question is not how often notes are written - it is whether
   * anybody drives Fredy through an LLM at all, and which of the three write paths they reach for.
   * A job created by interview is the expensive one to keep working, so it is worth knowing whether
   * it is ever used.
   */
  MCP_LISTING_NOTE_WRITTEN: 'MCP_LISTING_NOTE_WRITTEN',
  MCP_LISTING_WATCH_CHANGED: 'MCP_LISTING_WATCH_CHANGED',
  MCP_JOB_CREATED: 'MCP_JOB_CREATED',
  /*
   * An application letter was drafted - which is the dialog being opened, not the clipboard being
   * written: the letter is rendered server-side when the dialog fetches it, and the copy happens
   * in the browser afterwards. Counted apart per path because the two answer different questions:
   * the web one says whether people reach for the button at all, the MCP one says whether anybody
   * asks an assistant for a letter rather than opening Fredy.
   */
  APPLICATION_LETTER_DRAFTED: 'APPLICATION_LETTER_DRAFTED',
  MCP_APPLICATION_LETTER: 'MCP_APPLICATION_LETTER',
  APPLICANT_PROFILE_SAVED: 'APPLICANT_PROFILE_SAVED',
  APPLICATION_TEMPLATE_SAVED: 'APPLICATION_TEMPLATE_SAVED',
};
