...
describe('#homegate testsuite()', () => {
  it('should test homegate provider', async () => {
    ...
    expect(notificationObj.serviceName).toBe('homegate');
    notificationObj.payload.forEach((notify) => {
      /** check the actual structure **/
      expect(notify.id).toBeTypeOf('string');
      expect(notify.title).toBeTypeOf('string');
      expect(notify.link).toBeTypeOf('string');
      expect(notify.link).toContain('https://www.homegate.ch');
      ...
      if (notify.image != null) {
        ...
        expect(notify.image).toContain('homegate.ch');
      }
    });
  });
});
